...
在典型的现代 web 应用中，可能的构建过程包括：

* 捆绑和 缩减 JavaScript 和 CSS 文件。
* 运行工具以调用之前每个生成的绑定和缩减任务。
* 编译 Less 或 SASS 文件复制到 CSS。
* 编译 CoffeeScript 或 TypeScript 文件到 JavaScript。

任务运行器是一种自动执行这些常规开发任务的工具。 Visual Studio为两个流行的基于JavaScript的任务运行器提供了内置支持： [Gulp](https://gulpjs.com/)和[Grunt](using-grunt.md)。
...
|任务名称|描述|
|--- |--- |
|清理： js|使用 rimraf 节点删除模块删除 site.js 文件的缩减的版本的任务。|
|清理： css|使用 rimraf 节点删除模块删除 site.css 文件的缩减的版本的任务。|
...
4.  在此之下**任务**中**任务运行程序资源管理器**，右键单击**清理**，然后选择**运行**从弹出菜单。

    ![任务运行程序资源管理器清理任务](using-gulp/_static/04-TaskRunner-clean.png)

    **任务运行程序资源管理器**将创建名为的新选项卡**清理**和执行清理任务，如中定义*gulpfile.js*。

5.  右键单击**清理**任务，然后选择**绑定** > **构建之前**。

    ![任务运行程序资源管理器绑定 BeforeBuild](using-gulp/_static/05-TaskRunner-BeforeBuild.png)

    **构建之前**绑定将配置 clean 任务，以在每次生成项目之前自动运行。

使用**Task Runner Explorer**的绑定以*gulpfile.js*顶部的注释形式存储，仅在Visual Studio中有效。 不需要Visual Studio的替代方法是在*csproj*件中配置gulp任务的自动执行。例如，将这个放在 *.csproj*文件：
...
4.  在**任务运行程序资源管理器**，右键单击**第一个**，然后选择**运行**。
    ...
    输出文本显示。 若要查看基于常见方案的示例，请参阅[Gulp 食谱](#gulp-recipes)。

## <a name="defining-and-running-tasks-in-a-series"></a>定义和运行系列任务

当你运行多个任务时，默认情况下任务将同时运行。 但是，如果你需要以特定顺序运行任务，你必须指定每个任务过程何时完成，以及为哪些任务依赖于另一个任务完成。
...
1.  在**任务运行程序资源管理器**，验证**min**任务已设置为运行**构建之前**。
...
## <a name="gulp-recipes"></a>Gulp 食谱

Gulp 社区提供 Gulp[食谱](https://github.com/gulpjs/gulp/blob/master/docs/recipes/README.md)。 这些食谱包含 Gulp 任务用于针对常见情况。
