...
Visual Studio**浏览器请求的自动生成**不起作用体验[IIS 进程托管模型](xref:fundamentals/servers/aspnet-core-module#in-process-hosting-model)。 使用进程内承载时，必须手动重新生成项目。 此体验的改进计划未来版本的 Visual Studio。
