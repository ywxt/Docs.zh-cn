...
ASP.NET Core 2.1 提供[ASP.NET Core标识](xref:security/authentication/identity)作为[Razor 类库](xref:razor-pages/ui-class)(RCL)。
...
* 删除现有的标识 2.0 代码和[基架标识](xref:security/authentication/scaffold-identity)到你的项目。 你的项目将使用[ASP.NET Core标识](xref:security/authentication/identity) [Razor 类库](xref:razor-pages/ui-class)。 你可以生成代码和 UI 的任何标识 UI 代码修改。 将你的代码更改应用到新的基架的 UI 代码。
...
本部分概述的步骤，以替换与的 ASP.NET Core 2.0 模板生成标识代码[ASP.NET Core标识](xref:security/authentication/identity) [Razor 类库](xref:razor-pages/ui-class)。 以下步骤对于 Razor 页项目，但 MVC 项目的方法是类似。
