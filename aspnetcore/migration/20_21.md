...
* 替换的包引用`Microsoft.AspNetCore.All`使用的包引用`Microsoft.AspNetCore.App`。 可能需要添加依赖项已从`Microsoft.AspNetCore.All`。 有关详细信息，请参阅 <xref:fundamentals/metapackage#migrate> 和 <xref:fundamentals/metapackage-app>。 对于面向.NET Framework 的应用程序：
  * 添加单独包引用，而不是元包引用。
  * 更新到 2.1 每个包引用。
...
## <a name="rules-for-projects-targeting-the-shared-runtime"></a>面向共享运行时的项目的规则
...
* 使用`Microsoft.AspNetCore.App`使用 Entity Framework Core 2.1.1 2.1.0 导致 NuGet 还原失败，由于包冲突。 建议的解决方案是升级`Microsoft.AspNetCore.App`到 2.1.1。 有关详细信息，请参阅[Microsoft.AspNetCore.App 与共享依赖项的包不能引用的修补程序版本](https://github.com/aspnet/Universe/issues/1180)。
...
* 使用无需更改标识 UI 2.0 代码。 完全支持使用标识 UI 2.0 代码。 这是一个不错的方法时生成的标识代码进行了重要更改。
...
* 删除标识基架生成`IdentityDbContext`derrived 类中的*领域/Identity/Data/* 文件夹。
...
* *_Validationscriptspartial.cshtml*移动到*Pages/Shared/_ValidationScriptsPartial.cshtml*
