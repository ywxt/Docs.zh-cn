...
* 项目引用`Microsoft.AspNetCore.All`或`Microsoft.AspNetCore.App`包，必须引用`Microsoft.Net.Sdk.Web`Sdk。
* 项目引用包或间接引用的项目`Microsoft.AspNetCore.All`或`Microsoft.AspNetCore.App`:
  * 必须引用`Microsoft.Net.Sdk.Web`Sdk，
  * 必须引用同一个共享的运行时包。 例如，如果 LibraryA 引用`Microsoft.AspNetCore.App`，引用它的任何项目必须引用`Microsoft.AspNetCore.App`。
* 可执行项目：
  * 是包含使用启动的应用的项目`dotnet run`。
  * 是应用程序运行和测试应用程序的项目。
  * 可执行的项目不能指定版本。 Sdk 指定隐式通过版本`<PackageReference Include="Microsoft.AspNetCore.App" />`。
* 被引用的项目，这就是：
  * 项目不是入口点，并
  * 项目引用`Microsoft.AspNetCore.All`或`Microsoft.AspNetCore.App`。
  * 被引用的项目必须指定包版本。
...
ASP.NET 核心 2.1 提供[ASP.NET 核心标识](xref:security/authentication/identity)作为[Razor 类库](xref:razor-pages/ui-class)。 如果您尚未做大量更改 2.0 模板生成的标识代码，请考虑以下的升级方法：
