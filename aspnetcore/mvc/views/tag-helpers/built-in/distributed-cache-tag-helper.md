...
ASP.NET Core 中内置了 `IDistributedCache` 的两个实现。 一个基于 SQL Server，另一个基于 Redis。 有关这些实现的详细信息，请参阅 <xref:performance/caching/distributed>。 这两个实现都涉及在 ASP.NET Core 的 startup.cs 中设置 `IDistributedCache` 的实例。
