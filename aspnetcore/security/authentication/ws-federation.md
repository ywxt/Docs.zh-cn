...
---
# <a name="authenticate-users-with-ws-federation-in-aspnet-core"></a>使用 WS 联合身份验证在 ASP.NET Core中的用户进行身份验证
...
* 输入信赖方的显示名称。 名称并不重要到 ASP.NET Core应用程序。
...
* ASP.NET Core标识需要**名称 ID**声明。 添加一个从**编辑声明规则**对话框：
...
* 输入应用程序注册的名称。 这并不重要到 ASP.NET Core应用程序。
...
## <a name="add-ws-federation-as-an-external-login-provider-for-aspnet-core-identity"></a>为 ASP.NET Core标识的外部登录提供程序中添加 WS 联合身份验证
...
## <a name="use-ws-federation-without-aspnet-core-identity"></a>使用 WS 联合身份验证而无需 ASP.NET Core标识
