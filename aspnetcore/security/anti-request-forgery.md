...
除了其中用户选择按钮以提交该表单的方案中，恶意的站点无法：

* 运行自动提交该表单的脚本。
* 将窗体提交作为 AJAX 请求中发送。
* 隐藏使用 CSS 窗体。

这些备用方案无需任何操作或从用户而不是最初访问恶意网站的输入。
...
> [!NOTE]
> [Razor 页](xref:razor-pages/index)会自动防范 XSRF/CSRF。 有关详细信息，请参阅[XSRF/CSRF 和 Razor 页](xref:razor-pages/index#xsrf)。
