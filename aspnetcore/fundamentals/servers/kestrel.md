...
Kestrel 是跨平台 [ASP.NET Core Web 服务器](xref:fundamentals/servers/index)，它基于 [libuv](https://github.com/libuv/libuv)（一个跨平台异步 I/O 库）。 Kestrel 是 Web 服务器，默认包括在 ASP.NET Core 项目模板中。
...
* [dev-certs tool](https://github.com/aspnet/DotNetTools/tree/dev/src/dotnet-dev-certs) 用于创建证书。
...
* `serverOptions.Configure(context.Configuration.GetSection("Kestrel"))` 通过 `.Endpoint(string name, options => { })` 方法返回 `KestrelConfigurationLoader`，可以用于补充已配置的终结点设置：
  ...
  也可以直接访问 `KestrelServerOptions.ConfigurationLoader` 来保持现有加载程序上的迭代，例如由 `WebHost.CreatedDeafaultBuilder` 提供的加载程序。
