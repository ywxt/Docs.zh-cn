...
## <a name="install-the-aspnet-core-module"></a>安装 ASP.NET Core 模块

ASP.NET Core 模块是一个 IIS 7.5+ 模块，它负责 ASP.NET Core HTTP 侦听器的进程管理，并将请求代理到它所管理的进程。 目前，为 IIS 安装 ASP.NET Core 模块的过程为手动操作。 在常规（而不是 Nano）计算机上安装 [.NET Core 托管捆绑包](xref:host-and-deploy/iis/index#install-the-net-core-hosting-bundle)。 在常规计算机上安装捆绑包之后，将以下文件复制到之前创建的文件共享。
