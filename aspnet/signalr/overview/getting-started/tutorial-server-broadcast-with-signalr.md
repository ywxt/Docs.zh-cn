...
---
<a name="tutorial-server-broadcast-with-signalr-2"></a>教程：使用 signalr 2 实现服务器广播
====================
...
在本教程中，将创建的实时应用程序想要定期"推送"的具有代表性的股票行情自动收录器应用程序或广播到所有连接的客户端将通知从服务器。 在本教程的第一部分，将从零开始创建该应用程序的简化的版本。 在本教程的其余部分，将安装 NuGet 包，它包含其他功能，查看这些功能的代码。
...
[Microsoft.AspNet.SignalR.Sample](http://nuget.org/packages/microsoft.aspnet.signalr.sample) NuGet 包安装 Visual Studio 项目中的示例模拟股票行情自动收录器应用程序。

> [!NOTE]
> 如果不想要逐步完成构建应用程序的步骤，你可以在新的空的 ASP.NET Web 应用程序项目中安装 SignalR.Sample 包。 如果不执行本教程中的步骤安装 NuGet 包**必须遵循 readme.txt 文件中的说明**。 若要运行包需要添加 OWIN 启动类的调用中已安装的程序包 ConfigureSignalR 方法。 如果不添加 OWIN 启动类，将收到错误。
...
在开始之前，请确保已在计算机上安装的 Visual Studio 2013。 如果未安装 Visual Studio，请参阅[ASP.NET 下载](https://www.asp.net/downloads)若要获取免费的 Visual Studio 2013 Express。
...
4. 在中**新的 ASP.NET**项目窗口中，保留**空**选中然后单击**创建项目**。
...
1. 在中**解决方案资源管理器**，右键单击项目，然后单击**添加 |SignalR Hub 类 (v2)**。
2. 命名新集线器*StockTickerHub.cs*，然后单击**添加**。 SignalR NuGet 包将添加到你的项目。
3. 模板代码替换为以下代码：

    [!code-csharp[Main](tutorial-server-broadcast-with-signalr/samples/sample2.cs)]

    [中心](https://msdn.microsoft.com/library/microsoft.aspnet.signalr.hub(v=vs.111).aspx)类用于定义客户端可以调用在服务器的方法。 要定义一种方法： `GetAllStocks()`。 当客户端最初连接到服务器时，它将调用此方法以获取所有其当前价格股票的列表。 该方法可以同步执行并返回`IEnumerable<Stock>`由于它会从内存中返回数据。 如果该方法必须通过执行将涉及等待，如数据库查找或 web 服务调用，获取的数据则会指定`Task<IEnumerable<Stock>>`作为返回值以启用异步处理。 有关详细信息，请参阅[ASP.NET SignalR 中心 API 指南-服务器-时要异步执行](../guide-to-the-api/hubs-api-guide-server.md#asyncmethods)。
...
4. 在项目文件夹中创建一个新类文件，将其命名*StockTicker.cs*，然后使用以下代码替换模板代码：
...
3. > [!IMPORTANT]
   > 请确保在 JavaScript 文件中引用*StockTicker.html*正确无误。 也就是说，确保你的脚本标记 (1.10.2 在示例中) 中的 jQuery 版本是在项目中的 jQuery 版本相同*脚本*文件夹，并确保你的脚本标记中的 SignalR 版本是 SignalR 相同在项目的版本*脚本*文件夹。 如有必要，请更改脚本标记中的文件名称。
...
StockTicker 应用程序的情况下，安装[Microsoft.AspNet.SignalR.Sample](http://nuget.org/packages/microsoft.aspnet.signalr.sample) NuGet 包包含更多的功能比只是创建从零开始的简化版本。 在本教程的此部分中，安装 NuGet 包并查看新功能和实现它们的代码。 如果无需执行本教程的前面的步骤安装包，您必须将 OWIN 启动类添加到你的项目。 此步骤是在 NuGet 包的 readme.txt 文件中所述。
...
3. 在中**解决方案资源管理器**，展开*SignalR.Sample*安装 SignalR.Sample 包创建的文件夹。
4. 在中*SignalR.Sample*文件夹中，右键单击*StockTicker.html*，然后单击**设为起始页**。
