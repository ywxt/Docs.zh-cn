...
> 
> <span data-ttu-id="4973b-131">你可以阅读更多有关中的 HTML 编码[使用窗体](https://go.microsoft.com/fwlink/?LinkId=202892)。</span><span class="sxs-lookup"><span data-stu-id="4973b-131">You can read more about HTML encoding in [Working with Forms](https://go.microsoft.com/fwlink/?LinkId=202892).</span></span>
...
<span data-ttu-id="4973b-170">在浏览器中显示的结果 (单击后**提交**):</span><span class="sxs-lookup"><span data-stu-id="4973b-170">The result displayed in a browser (after clicking **Submit**):</span></span>
...
<span data-ttu-id="4973b-205">但首先，一点的技术背景知识。</span><span class="sxs-lookup"><span data-stu-id="4973b-205">But first, a little technical background.</span></span>
...
### <a name="code-and-markup-comments"></a><span data-ttu-id="4973b-270">代码 （和标记） 的注释</span><span class="sxs-lookup"><span data-stu-id="4973b-270">Code (and Markup) Comments</span></span>
...
<span data-ttu-id="4973b-300">声明变量使用`var`关键字 （如果不想指定类型） 或使用的类型名称：</span><span class="sxs-lookup"><span data-stu-id="4973b-300">You declare a variable using the `var` keyword (if you don't want to specify a type) or by using the name of the type:</span></span>
...
<span data-ttu-id="4973b-317">下表列出了一些常见的转换和测试方法的变量。</span><span class="sxs-lookup"><span data-stu-id="4973b-317">The following table lists some common conversion and test methods for variables.</span></span>
...
- <span data-ttu-id="4973b-392">`myImagesFolder`: `http://myserver/myapp/images`</span><span class="sxs-lookup"><span data-stu-id="4973b-392">`myImagesFolder`: `http://myserver/myapp/images`</span></span>
...
<span data-ttu-id="4973b-417">在浏览器中显示的最后两个条件块的结果：</span><span class="sxs-lookup"><span data-stu-id="4973b-417">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor-Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="4973b-419">循环的代码</span><span class="sxs-lookup"><span data-stu-id="4973b-419">Looping Code</span></span>

<span data-ttu-id="4973b-420">通常需要反复运行相同的语句。</span><span class="sxs-lookup"><span data-stu-id="4973b-420">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="4973b-421">通过循环执行此操作。</span><span class="sxs-lookup"><span data-stu-id="4973b-421">You do this by looping.</span></span> <span data-ttu-id="4973b-422">例如，您通常运行同一个语句为每个项集合中的数据。</span><span class="sxs-lookup"><span data-stu-id="4973b-422">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="4973b-423">如果你确切地知道多少次循环，则可以使用`for`循环。</span><span class="sxs-lookup"><span data-stu-id="4973b-423">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="4973b-424">此种类型的循环非常适合计算或向下计数：</span><span class="sxs-lookup"><span data-stu-id="4973b-424">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="4973b-425">循环开头`for`关键字后, 跟三个语句在括号内，每个以分号结尾。</span><span class="sxs-lookup"><span data-stu-id="4973b-425">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="4973b-426">在括号内的第一个语句 (`var i=10;`) 创建一个计数器并将初始化为 10。</span><span class="sxs-lookup"><span data-stu-id="4973b-426">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="4973b-427">无需命名计数器`i`&#8212;可以使用任何变量。</span><span class="sxs-lookup"><span data-stu-id="4973b-427">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="4973b-428">当`for`循环运行时，会自动增加计数器的数值。</span><span class="sxs-lookup"><span data-stu-id="4973b-428">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="4973b-429">第二个语句 (`i < 21;`) 设置距离您要计数的条件。</span><span class="sxs-lookup"><span data-stu-id="4973b-429">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="4973b-430">在这种情况下，想要转到最多 20 个 （即，继续时该计数器是小于 21）。</span><span class="sxs-lookup"><span data-stu-id="4973b-430">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="4973b-431">第三个语句 (`i++` ) 使用递增运算符，只需指定计数器应具有 1 添加到其每次循环运行的时。</span><span class="sxs-lookup"><span data-stu-id="4973b-431">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="4973b-432">大括号内是循环的每次迭代就会运行的代码。</span><span class="sxs-lookup"><span data-stu-id="4973b-432">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="4973b-433">标记将创建一个新段落 (`<p>`元素) 每个时间，并将行添加到输出，显示的值`i`（计数器）。</span><span class="sxs-lookup"><span data-stu-id="4973b-433">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="4973b-434">在运行此页时，该示例将创建显示每个行，该值指示项编号中的文本的输出的 11 行。</span><span class="sxs-lookup"><span data-stu-id="4973b-434">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor-Img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="4973b-436">如果您正在使用集合或数组，通常使用`foreach`循环。</span><span class="sxs-lookup"><span data-stu-id="4973b-436">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="4973b-437">集合是一组类似对象和`foreach`循环可让你执行在集合中每一项任务。</span><span class="sxs-lookup"><span data-stu-id="4973b-437">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="4973b-438">此类型的循环非常方便的集合，因为与不同`for`循环中，您无需使计数器递增或设置的限制。</span><span class="sxs-lookup"><span data-stu-id="4973b-438">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="4973b-439">相反，`foreach`循环代码只需继续遍历该集合直到完成。</span><span class="sxs-lookup"><span data-stu-id="4973b-439">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="4973b-440">例如，以下代码将返回中的项`Request.ServerVariables`集合，这是一个对象，包含有关你的 web 服务器的信息。</span><span class="sxs-lookup"><span data-stu-id="4973b-440">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="4973b-441">它使用`foreac`h 循环，以显示每个项的名称，通过创建一个新`<li>`HTML 项目符号列表中的元素。</span><span class="sxs-lookup"><span data-stu-id="4973b-441">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="4973b-442">`foreach`关键字后跟括号，声明表示集合中的单个项的变量 (在示例中， `var item`) 后, 跟`in`关键字后, 跟想要循环访问的集合。</span><span class="sxs-lookup"><span data-stu-id="4973b-442">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="4973b-443">正文中`foreach`循环中，您可以访问使用你之前声明的变量的当前项。</span><span class="sxs-lookup"><span data-stu-id="4973b-443">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor-Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="4973b-445">若要创建一个更通用的循环，请使用`while`语句：</span><span class="sxs-lookup"><span data-stu-id="4973b-445">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="4973b-446">一个`while`循环开头`while`关键字后, 跟括号指定循环继续的时间长度 (这里，为只要`countNum`小于 50)，然后要重复的块。</span><span class="sxs-lookup"><span data-stu-id="4973b-446">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="4973b-447">循环通常递增 （将添加到） 或递减 （从减） 的变量或用于计数的对象。</span><span class="sxs-lookup"><span data-stu-id="4973b-447">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="4973b-448">在示例中，`+=`运算符将添加到 1`countNum`每次循环运行的时。</span><span class="sxs-lookup"><span data-stu-id="4973b-448">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="4973b-449">(若要递减的向下计数的循环中的变量，将使用递减运算符`-=`)。</span><span class="sxs-lookup"><span data-stu-id="4973b-449">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="4973b-450">对象和集合</span><span class="sxs-lookup"><span data-stu-id="4973b-450">Objects and Collections</span></span>

<span data-ttu-id="4973b-451">在 ASP.NET 网站中几乎所有内容是一个对象，包括在 web 页面。</span><span class="sxs-lookup"><span data-stu-id="4973b-451">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="4973b-452">本部分介绍一些重要的对象，您将使用频繁地在代码中。</span><span class="sxs-lookup"><span data-stu-id="4973b-452">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="4973b-453">页对象</span><span class="sxs-lookup"><span data-stu-id="4973b-453">Page Objects</span></span>

<span data-ttu-id="4973b-454">在 ASP.NET 中的最基本对象是页。</span><span class="sxs-lookup"><span data-stu-id="4973b-454">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="4973b-455">可以访问的不符合要求的任何对象而直接的 page 对象的属性。</span><span class="sxs-lookup"><span data-stu-id="4973b-455">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="4973b-456">以下代码获取页的文件路径，使用`Request`页的对象：</span><span class="sxs-lookup"><span data-stu-id="4973b-456">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="4973b-457">若要使其明确的引用的当前的页对象上属性和方法，你可以选择使用关键字`this`来表示在代码中的页对象。</span><span class="sxs-lookup"><span data-stu-id="4973b-457">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="4973b-458">下面是前面的代码示例中，使用`this`添加来表示页面：</span><span class="sxs-lookup"><span data-stu-id="4973b-458">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="4973b-459">可以使用的属性`Page`对象以获得大量的信息，例如：</span><span class="sxs-lookup"><span data-stu-id="4973b-459">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="4973b-460">`Request`。</span><span class="sxs-lookup"><span data-stu-id="4973b-460">`Request`.</span></span> <span data-ttu-id="4973b-461">如您所见，这是信息的一系列有关当前请求，包括哪种类型的浏览器发出了请求、 页面、 用户标识，等等的 URL。</span><span class="sxs-lookup"><span data-stu-id="4973b-461">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="4973b-462">`Response`。</span><span class="sxs-lookup"><span data-stu-id="4973b-462">`Response`.</span></span> <span data-ttu-id="4973b-463">这是信息的有关在服务器代码完成运行时将发送到浏览器的响应 （页） 集合。</span><span class="sxs-lookup"><span data-stu-id="4973b-463">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="4973b-464">例如，此属性可用于将信息写入到响应。</span><span class="sxs-lookup"><span data-stu-id="4973b-464">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="4973b-465">集合对象 （数组和字典）</span><span class="sxs-lookup"><span data-stu-id="4973b-465">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="4973b-466">一个*集合*是一组相同的类型，如集合的对象的`Customer`数据库中的对象。</span><span class="sxs-lookup"><span data-stu-id="4973b-466">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="4973b-467">ASP.NET 包含多个内置集合，例如`Request.Files`集合。</span><span class="sxs-lookup"><span data-stu-id="4973b-467">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="4973b-468">通常将使用集合中的数据。</span><span class="sxs-lookup"><span data-stu-id="4973b-468">You'll often work with data in collections.</span></span> <span data-ttu-id="4973b-469">两个常见集合类型是*数组*并*字典*。</span><span class="sxs-lookup"><span data-stu-id="4973b-469">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="4973b-470">如果你想要存储类似项的集合，但不想创建一个单独的变量来保存每个项，数组非常有用：</span><span class="sxs-lookup"><span data-stu-id="4973b-470">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="4973b-471">对于数组，声明特定数据类型，如`string`， `int`，或`DateTime`。</span><span class="sxs-lookup"><span data-stu-id="4973b-471">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="4973b-472">若要指示该变量可以包含的数组，向声明添加方括号 (如`string[]`或`int[]`)。</span><span class="sxs-lookup"><span data-stu-id="4973b-472">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="4973b-473">您可以访问项在数组中使用它们的位置 （索引） 或通过使用`foreach`语句。</span><span class="sxs-lookup"><span data-stu-id="4973b-473">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="4973b-474">从零开始的数组索引为&#8212;，它是第一项是在位置 0，第二项是在位置 1，依此类推。</span><span class="sxs-lookup"><span data-stu-id="4973b-474">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="4973b-475">可以通过获取确定数组中的项的数目及其`Length`属性。</span><span class="sxs-lookup"><span data-stu-id="4973b-475">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="4973b-476">若要获取特定项的数组 （若要搜索数组） 中的位置，请使用`Array.IndexOf`方法。</span><span class="sxs-lookup"><span data-stu-id="4973b-476">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="4973b-477">此外可以执行诸如反向数组的内容 (`Array.Reverse`方法) 或对内容进行排序 (`Array.Sort`方法)。</span><span class="sxs-lookup"><span data-stu-id="4973b-477">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="4973b-478">在浏览器中显示的字符串数组代码的输出：</span><span class="sxs-lookup"><span data-stu-id="4973b-478">The output of the string array code displayed in a browser:</span></span>

![Razor-Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="4973b-480">字典是键/值对的集合，其中提供密钥 （或名称） 设置或检索相应的值：</span><span class="sxs-lookup"><span data-stu-id="4973b-480">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="4973b-481">若要创建一个字典，请使用`new`关键字来指示您要创建新的字典对象。</span><span class="sxs-lookup"><span data-stu-id="4973b-481">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="4973b-482">可以将字典分配给变量使用`var`关键字。</span><span class="sxs-lookup"><span data-stu-id="4973b-482">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="4973b-483">指示使用尖括号字典中的项的数据类型 ( `< >` )。</span><span class="sxs-lookup"><span data-stu-id="4973b-483">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="4973b-484">在声明的末尾，必须添加一对圆括号，因为这是实际创建一个新的字典的方法。</span><span class="sxs-lookup"><span data-stu-id="4973b-484">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="4973b-485">若要将项添加到字典中，可以调用`Add`字典变量或方法 (`myScores`这种情况下)，然后指定一个键和值。</span><span class="sxs-lookup"><span data-stu-id="4973b-485">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="4973b-486">或者，可以使用方括号以指示该密钥，并执行简单的赋值，如以下示例所示：</span><span class="sxs-lookup"><span data-stu-id="4973b-486">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="4973b-487">若要从字典中获取一个值，请在方括号中指定密钥：</span><span class="sxs-lookup"><span data-stu-id="4973b-487">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="4973b-488">调用带参数的方法</span><span class="sxs-lookup"><span data-stu-id="4973b-488">Calling Methods with Parameters</span></span>

<span data-ttu-id="4973b-489">在阅读本文前面部分中，带有程序的对象可以具有方法。</span><span class="sxs-lookup"><span data-stu-id="4973b-489">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="4973b-490">例如，`Database`对象可能具有`Database.Connect`方法。</span><span class="sxs-lookup"><span data-stu-id="4973b-490">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="4973b-491">许多方法还具有一个或多个参数。</span><span class="sxs-lookup"><span data-stu-id="4973b-491">Many methods also have one or more parameters.</span></span> <span data-ttu-id="4973b-492">一个*参数*是向方法传递一个值以使该方法以完成其任务。</span><span class="sxs-lookup"><span data-stu-id="4973b-492">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="4973b-493">例如，查看有关声明`Request.MapPath`方法，它使用三个参数：</span><span class="sxs-lookup"><span data-stu-id="4973b-493">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="4973b-494">（已换行以使其更具可读性。</span><span class="sxs-lookup"><span data-stu-id="4973b-494">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="4973b-495">请记住，可以将分行符几乎任何位置除了内部字符串用引号括起来。)</span><span class="sxs-lookup"><span data-stu-id="4973b-495">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="4973b-496">此方法相对应的服务器上的物理路径返回到指定的虚拟路径。</span><span class="sxs-lookup"><span data-stu-id="4973b-496">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="4973b-497">该方法的三个参数是`virtualPath`， `baseVirtualDir`，和`allowCrossAppMapping`。</span><span class="sxs-lookup"><span data-stu-id="4973b-497">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="4973b-498">（请注意，在声明中，所列的参数将接受的数据的数据类型）。当调用此方法时，必须提供所有三个参数的值。</span><span class="sxs-lookup"><span data-stu-id="4973b-498">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="4973b-499">Razor 语法可以用于将参数传递给方法的两个选项：*位置参数*并*命名参数*。</span><span class="sxs-lookup"><span data-stu-id="4973b-499">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="4973b-500">若要调用使用位置参数的方法，请按严格顺序方法声明中指定的传递参数。</span><span class="sxs-lookup"><span data-stu-id="4973b-500">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="4973b-501">（您通常知道此顺序通过阅读文档的方法。）必须遵循的顺序，并且你不能跳过的任何参数&#8212;如果有必要，请传递了空字符串 (`""`) 或`null`不具有的值为位置参数。</span><span class="sxs-lookup"><span data-stu-id="4973b-501">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="4973b-502">下面的示例假定你有一个名为的文件夹*脚本*上你的网站。</span><span class="sxs-lookup"><span data-stu-id="4973b-502">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="4973b-503">该代码调用`Request.MapPath`方法，并传递正确的顺序中的三个参数的值。</span><span class="sxs-lookup"><span data-stu-id="4973b-503">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="4973b-504">然后，它显示生成映射的路径。</span><span class="sxs-lookup"><span data-stu-id="4973b-504">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="4973b-505">当某个方法具有多个参数时，您可以保留你的代码更具可读性使用命名参数。</span><span class="sxs-lookup"><span data-stu-id="4973b-505">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="4973b-506">若要调用的方法使用命名的参数，您指定的参数名称后跟一个冒号 （:），然后选择值。</span><span class="sxs-lookup"><span data-stu-id="4973b-506">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="4973b-507">命名参数的优点是，可以将它们传递所需的任何顺序。</span><span class="sxs-lookup"><span data-stu-id="4973b-507">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="4973b-508">（一个缺点是方法调用不是精简。）</span><span class="sxs-lookup"><span data-stu-id="4973b-508">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="4973b-509">下面的示例调用按上面所述的相同方法，但使用命名参数提供的值：</span><span class="sxs-lookup"><span data-stu-id="4973b-509">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="4973b-510">正如您所看到的会以不同的顺序传递参数。</span><span class="sxs-lookup"><span data-stu-id="4973b-510">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="4973b-511">但是，如果在运行上面的例子和此示例中，它们将返回相同的值。</span><span class="sxs-lookup"><span data-stu-id="4973b-511">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="4973b-512">处理错误</span><span class="sxs-lookup"><span data-stu-id="4973b-512">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="4973b-513">Try Catch 语句</span><span class="sxs-lookup"><span data-stu-id="4973b-513">Try-Catch Statements</span></span>

<span data-ttu-id="4973b-514">通常必须在代码中可能会失败的原因有您的控制范围内的语句。</span><span class="sxs-lookup"><span data-stu-id="4973b-514">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="4973b-515">例如：</span><span class="sxs-lookup"><span data-stu-id="4973b-515">For example:</span></span>

- <span data-ttu-id="4973b-516">如果你的代码尝试创建或访问的文件时，可能会出现各种类型的错误。</span><span class="sxs-lookup"><span data-stu-id="4973b-516">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="4973b-517">所需的文件可能不存在，则可能锁定，代码可能不具有的权限，等等。</span><span class="sxs-lookup"><span data-stu-id="4973b-517">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="4973b-518">同样，如果你的代码尝试更新数据库中的记录，可能出现权限问题、 与数据库的连接可能会断开，要保存的数据可能无效，依此类推。</span><span class="sxs-lookup"><span data-stu-id="4973b-518">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="4973b-519">在编程术语中，这些应用场景被称为*异常*。</span><span class="sxs-lookup"><span data-stu-id="4973b-519">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="4973b-520">如果你的代码遇到的异常，它将生成 （的引发操作） 的错误消息的在最令人讨厌的用户：</span><span class="sxs-lookup"><span data-stu-id="4973b-520">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor-Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="4973b-522">在其中您的代码可能会发生异常的情况下，为了避免这种类型的错误消息，可以使用`try/catch`语句。</span><span class="sxs-lookup"><span data-stu-id="4973b-522">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="4973b-523">在`try`语句中，您运行要检查的代码。</span><span class="sxs-lookup"><span data-stu-id="4973b-523">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="4973b-524">一个或多个`catch`语句，则可以寻找特定于可能发生的错误 （特定类型的异常）。</span><span class="sxs-lookup"><span data-stu-id="4973b-524">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="4973b-525">可以包括任意多个`catch`语句作为您需要查找你预期会发生的错误。</span><span class="sxs-lookup"><span data-stu-id="4973b-525">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="4973b-526">我们建议您不要使用`Response.Redirect`中的方法`try/catch`语句，因为它可以在页面中导致异常。</span><span class="sxs-lookup"><span data-stu-id="4973b-526">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>


<span data-ttu-id="4973b-527">下面的示例显示了创建第一个请求上的文本文件，然后显示一个按钮，使用户可以打开文件的页面。</span><span class="sxs-lookup"><span data-stu-id="4973b-527">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="4973b-528">该示例故意使用错误的文件名称，以便它将导致异常。</span><span class="sxs-lookup"><span data-stu-id="4973b-528">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="4973b-529">该代码包括`catch`两个可能的异常的语句： `FileNotFoundException`，就会出现此错误，文件名称是否和`DirectoryNotFoundException`，如果 ASP.NET 甚至找不到该文件夹将出现此情况。</span><span class="sxs-lookup"><span data-stu-id="4973b-529">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="4973b-530">（您可以取消注释该示例中的语句才能看到它时一切运行正常的运行。）</span><span class="sxs-lookup"><span data-stu-id="4973b-530">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="4973b-531">如果你的代码未处理异常，会看到一个错误页面，如前面的屏幕截图。</span><span class="sxs-lookup"><span data-stu-id="4973b-531">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="4973b-532">但是，`try/catch`部分可帮助防止用户看到这些类型的错误。</span><span class="sxs-lookup"><span data-stu-id="4973b-532">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="4973b-533">其他资源</span><span class="sxs-lookup"><span data-stu-id="4973b-533">Additional Resources</span></span>

<span data-ttu-id="4973b-534">**使用 Visual Basic 进行编程**</span><span class="sxs-lookup"><span data-stu-id="4973b-534">**Programming with Visual Basic**</span></span>


[<span data-ttu-id="4973b-535">附录： Visual Basic 语言和语法</span><span class="sxs-lookup"><span data-stu-id="4973b-535">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)


<span data-ttu-id="4973b-536">**参考文档**</span><span class="sxs-lookup"><span data-stu-id="4973b-536">**Reference Documentation**</span></span>


[<span data-ttu-id="4973b-537">ASP.NET 2.0</span><span class="sxs-lookup"><span data-stu-id="4973b-537">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="4973b-538">C# 语言</span><span class="sxs-lookup"><span data-stu-id="4973b-538">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
