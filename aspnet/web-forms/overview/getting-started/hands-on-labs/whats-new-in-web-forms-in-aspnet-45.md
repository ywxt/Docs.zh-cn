...
- <span data-ttu-id="16f1e-127">使用强类型化数据绑定表达式</span><span class="sxs-lookup"><span data-stu-id="16f1e-127">Use strongly-typed data-binding expressions</span></span>
...
<span data-ttu-id="16f1e-143">本动手实验包括以下练习：</span><span class="sxs-lookup"><span data-stu-id="16f1e-143">This hands-on lab includes the following exercises:</span></span>
...
6. <span data-ttu-id="16f1e-176">按**F5**以运行该解决方案，转到**客户**页面以查看在操作中的转发器。</span><span class="sxs-lookup"><span data-stu-id="16f1e-176">Press **F5** to run the solution and go to the **Customers** page to see the repeater in action.</span></span> <span data-ttu-id="16f1e-177">解决方案是使用 CodeFirst，将创建此数据库，并填充本地 SQL Express 实例中运行应用程序时。</span><span class="sxs-lookup"><span data-stu-id="16f1e-177">As the solution is using CodeFirst, the database will be created and populated in your local SQL Express instance when running the application.</span></span>
    ...
    <span data-ttu-id="16f1e-178">![列出与中继器客户](whats-new-in-web-forms-in-aspnet-45/_static/image1.png "列出与中继器客户")</span><span class="sxs-lookup"><span data-stu-id="16f1e-178">![Listing the customers with a repeater](whats-new-in-web-forms-in-aspnet-45/_static/image1.png "Listing the customers with a repeater")</span></span>
...
6. <span data-ttu-id="16f1e-222">按**F5**能够开始调试站点并转到产品页。</span><span class="sxs-lookup"><span data-stu-id="16f1e-222">Press **F5** to start debugging the site and go to the Products page.</span></span> <span data-ttu-id="16f1e-223">你应看到 GridView 中填充了 GetCategories 方法返回的类别。</span><span class="sxs-lookup"><span data-stu-id="16f1e-223">You should see that the GridView is populated with the categories returned by the GetCategories method.</span></span>
    ...
    <span data-ttu-id="16f1e-225">*填充使用模型绑定的 GridView*</span><span class="sxs-lookup"><span data-stu-id="16f1e-225">*Populating a GridView using model binding*</span></span>
...
<span data-ttu-id="16f1e-288">在上一任务中，选择数据的主要使用模型绑定，在此任务中，您将学习如何在更新操作中使用模型绑定。</span><span class="sxs-lookup"><span data-stu-id="16f1e-288">In the previous task, you have used model binding mainly for selecting data, in this task you will learn how to use model binding in update operations.</span></span>
...
3. <span data-ttu-id="16f1e-401">打开**Products.aspx**并添加**ValidationSummary**类别 GridView 以显示模型错误的列表下面的控件。</span><span class="sxs-lookup"><span data-stu-id="16f1e-401">Open **Products.aspx** and add a **ValidationSummary** control below the categories GridView to show the list of model errors.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample28.aspx)]
4. <span data-ttu-id="16f1e-402">运行该站点，并转到产品页。</span><span class="sxs-lookup"><span data-stu-id="16f1e-402">Run the site and go to the Products page.</span></span> <span data-ttu-id="16f1e-403">尝试更新使用重复的值的类别的名称。</span><span class="sxs-lookup"><span data-stu-id="16f1e-403">Try to update the name of a category using an duplicated value.</span></span>

    <span data-ttu-id="16f1e-404">请注意，处理了该异常和错误消息将出现在**ValidationSummary**控件。</span><span class="sxs-lookup"><span data-stu-id="16f1e-404">Notice that the exception was handled and the error message appears in the **ValidationSummary** control.</span></span>

    <span data-ttu-id="16f1e-405">![重复类别错误](whats-new-in-web-forms-in-aspnet-45/_static/image17.png "重复类别错误")</span><span class="sxs-lookup"><span data-stu-id="16f1e-405">![Duplicated category error](whats-new-in-web-forms-in-aspnet-45/_static/image17.png "Duplicated category error")</span></span>

    <span data-ttu-id="16f1e-406">*重复的类别错误*</span><span class="sxs-lookup"><span data-stu-id="16f1e-406">*Duplicated category error*</span></span>

<a id="Task_4_-_Request_Validation_in_ASPNET_Web_Forms_45"></a>
#### <a name="task-4---request-validation-in-aspnet-web-forms-45"></a><span data-ttu-id="16f1e-407">任务 4-请求在 ASP.NET Web 窗体 4.5 中的验证</span><span class="sxs-lookup"><span data-stu-id="16f1e-407">Task 4 - Request Validation in ASP.NET Web Forms 4.5</span></span>

<span data-ttu-id="16f1e-408">在 ASP.NET 中的请求验证功能可提供特定级别的针对跨站点脚本 (XSS) 攻击的默认保护。</span><span class="sxs-lookup"><span data-stu-id="16f1e-408">The request validation feature in ASP.NET provides a certain level of default protection against cross-site scripting (XSS) attacks.</span></span> <span data-ttu-id="16f1e-409">在以前版本的 ASP.NET，请求验证已按默认启用，仅可以为整个页面已禁用。</span><span class="sxs-lookup"><span data-stu-id="16f1e-409">In previous versions of ASP.NET, request validation was enabled by default and could only be disabled for an entire page.</span></span> <span data-ttu-id="16f1e-410">使用新版本的 ASP.NET Web 窗体可以现在禁用单个控件的请求验证、 执行延迟请求验证或访问 （但请注意如果这样做 ！） 的未经过验证的请求数据。</span><span class="sxs-lookup"><span data-stu-id="16f1e-410">With the new version of ASP.NET Web Forms you can now disable the request validation for a single control, perform lazy request validation or access un-validated request data (be careful if you do so!).</span></span>

1. <span data-ttu-id="16f1e-411">按**Ctrl + F5**以启动网站，而不进行调试，然后转到产品页。</span><span class="sxs-lookup"><span data-stu-id="16f1e-411">Press **Ctrl+F5** to start the site without debugging and go to the Products page.</span></span> <span data-ttu-id="16f1e-412">选择一个类别，然后单击**编辑**上产品中的任何链接。</span><span class="sxs-lookup"><span data-stu-id="16f1e-412">Select a category and then click the **Edit** link on any of the products.</span></span>
2. <span data-ttu-id="16f1e-413">键入包含具有潜在危险的内容、 对实例包括 HTML 标记的描述。</span><span class="sxs-lookup"><span data-stu-id="16f1e-413">Type a description containing potentially dangerous content, for instance including HTML tags.</span></span> <span data-ttu-id="16f1e-414">注意到由于请求验证引发的异常。</span><span class="sxs-lookup"><span data-stu-id="16f1e-414">Take notice of the exception thrown due to the request validation.</span></span>

    <span data-ttu-id="16f1e-415">![编辑具有潜在危险的内容的产品](whats-new-in-web-forms-in-aspnet-45/_static/image18.png "编辑具有潜在危险的内容的产品")</span><span class="sxs-lookup"><span data-stu-id="16f1e-415">![Editing a product with potentially dangerous content](whats-new-in-web-forms-in-aspnet-45/_static/image18.png "Editing a product with potentially dangerous content")</span></span>

    <span data-ttu-id="16f1e-416">*编辑具有潜在危险的内容的产品*</span><span class="sxs-lookup"><span data-stu-id="16f1e-416">*Editing a product with potentially dangerous content*</span></span>

    <span data-ttu-id="16f1e-417">![因请求验证而引发的异常](whats-new-in-web-forms-in-aspnet-45/_static/image19.png "因请求验证而引发的异常")</span><span class="sxs-lookup"><span data-stu-id="16f1e-417">![Exception thrown due to request validation](whats-new-in-web-forms-in-aspnet-45/_static/image19.png "Exception thrown due to request validation")</span></span>

    <span data-ttu-id="16f1e-418">*因请求验证而引发的异常*</span><span class="sxs-lookup"><span data-stu-id="16f1e-418">*Exception thrown due to request validation*</span></span>
3. <span data-ttu-id="16f1e-419">关闭页，然后在 Visual Studio 中，按**SHIFT + F5**来停止调试。</span><span class="sxs-lookup"><span data-stu-id="16f1e-419">Close the page and, in Visual Studio, press **SHIFT+F5** to stop debugging.</span></span>
4. <span data-ttu-id="16f1e-420">打开**ProductDetails.aspx**页然后找到**说明**文本框。</span><span class="sxs-lookup"><span data-stu-id="16f1e-420">Open the **ProductDetails.aspx** page and locate the **Description** TextBox.</span></span>
5. <span data-ttu-id="16f1e-421">添加新**ValidateRequestMode**到文本框的属性并将其值设置为**禁用**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-421">Add the new **ValidateRequestMode** property to the TextBox and set its value to **Disabled**.</span></span>

    <span data-ttu-id="16f1e-422">新**ValidateRequestMode**特性，您可以禁用每个控件梯度的请求验证。</span><span class="sxs-lookup"><span data-stu-id="16f1e-422">The new **ValidateRequestMode** attribute allows you to disable the request validation granularly on each control.</span></span> <span data-ttu-id="16f1e-423">当你想要使用的输入可能收到 HTML 代码，但想要保留用于网页的其余内容的验证时，这非常有用。</span><span class="sxs-lookup"><span data-stu-id="16f1e-423">This is useful when you want to use an input that may receive HTML code, but want to keep the validation working for the rest of the page.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample29.aspx)]
6. <span data-ttu-id="16f1e-424">按**F5**运行 web 应用程序。</span><span class="sxs-lookup"><span data-stu-id="16f1e-424">Press **F5** to run the web application.</span></span> <span data-ttu-id="16f1e-425">再次打开编辑产品页并完成包括 HTML 标记的产品说明。</span><span class="sxs-lookup"><span data-stu-id="16f1e-425">Open the edit product page again and complete a product description including HTML tags.</span></span> <span data-ttu-id="16f1e-426">请注意，你可以现在 HTML 内容添加到说明。</span><span class="sxs-lookup"><span data-stu-id="16f1e-426">Notice that you can now add HTML content to the description.</span></span>

    <span data-ttu-id="16f1e-427">![请求验证有关产品说明禁用](whats-new-in-web-forms-in-aspnet-45/_static/image20.png "请求产品说明为禁用的验证")</span><span class="sxs-lookup"><span data-stu-id="16f1e-427">![Request validation disabled for the product description](whats-new-in-web-forms-in-aspnet-45/_static/image20.png "Request validation disabled for the product description")</span></span>

    <span data-ttu-id="16f1e-428">*请求产品说明为禁用的验证*</span><span class="sxs-lookup"><span data-stu-id="16f1e-428">*Request validation disabled for the product description*</span></span>

    > [!NOTE]
    > <span data-ttu-id="16f1e-429">应在生产应用程序中，整理要确保输入仅安全的 HTML 标记的用户输入的 HTML 代码 (例如，有没有&lt;脚本&gt;标记)。</span><span class="sxs-lookup"><span data-stu-id="16f1e-429">In a production application, you should sanitize the HTML code entered by the user to make sure only safe HTML tags are entered (for example, there are no &lt;script&gt; tags).</span></span> <span data-ttu-id="16f1e-430">若要执行此操作，可以使用[Microsoft Web 保护库](https://www.nuget.org/packages/AntiXSS)。</span><span class="sxs-lookup"><span data-stu-id="16f1e-430">To do this, you can use [Microsoft Web Protection Library](https://www.nuget.org/packages/AntiXSS).</span></span>
7. <span data-ttu-id="16f1e-431">再次编辑产品。</span><span class="sxs-lookup"><span data-stu-id="16f1e-431">Edit the product again.</span></span> <span data-ttu-id="16f1e-432">在名称字段中键入 HTML 代码，然后单击**保存**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-432">Type HTML code in the Name field and click **Save**.</span></span> <span data-ttu-id="16f1e-433">请注意，说明字段中仅禁用请求验证，并且重新字段的其余部分仍验证针对潜在危险的内容。</span><span class="sxs-lookup"><span data-stu-id="16f1e-433">Notice that Request Validation is only disabled for the Description field and the rest of the fields re still validated against the potentially dangerous content.</span></span>

    <span data-ttu-id="16f1e-434">![请求验证在其余字段中启用](whats-new-in-web-forms-in-aspnet-45/_static/image21.png "请求在其余字段中启用的验证")</span><span class="sxs-lookup"><span data-stu-id="16f1e-434">![Request validation enabled in the rest of the fields](whats-new-in-web-forms-in-aspnet-45/_static/image21.png "Request validation enabled in the rest of the fields")</span></span>

    <span data-ttu-id="16f1e-435">*在其余字段中启用请求验证*</span><span class="sxs-lookup"><span data-stu-id="16f1e-435">*Request validation enabled in the rest of the fields*</span></span>

    <span data-ttu-id="16f1e-436">ASP.NET Web 窗体 4.5 包括新的请求验证模式，延迟执行请求验证。</span><span class="sxs-lookup"><span data-stu-id="16f1e-436">ASP.NET Web Forms 4.5 includes a new request validation mode to perform request validation lazily.</span></span> <span data-ttu-id="16f1e-437">使用请求验证模式设置为**4.5**，如果一种代码访问*Request.Form [&quot;密钥&quot;]*，ASP.NET 4.5 请求验证将仅触发请求验证该窗体集合中的特定元素。</span><span class="sxs-lookup"><span data-stu-id="16f1e-437">With the request validation mode set to **4.5**, if a piece of code accesses *Request.Form[&quot;key&quot;]*, ASP.NET 4.5's request validation will only trigger request validation for that specific element in the form collection.</span></span>

    <span data-ttu-id="16f1e-438">此外，ASP.NET 4.5 现在包括从 Microsoft 反 XSS 库 v4.0 的核心编码例程。</span><span class="sxs-lookup"><span data-stu-id="16f1e-438">Additionally, ASP.NET 4.5 now includes core encoding routines from the Microsoft Anti-XSS Library v4.0.</span></span> <span data-ttu-id="16f1e-439">编码例程实现新的反 XSS *AntiXssEncoder*类型位于新**System.Web.Security.AntiXss**命名空间。</span><span class="sxs-lookup"><span data-stu-id="16f1e-439">The Anti-XSS encoding routines are implemented by the new *AntiXssEncoder* type found in the new **System.Web.Security.AntiXss** namespace.</span></span> <span data-ttu-id="16f1e-440">与**encoderType**参数配置为使用*AntiXssEncoder*，所有输出自动编码在 ASP.NET 内使用的新的编码例程。</span><span class="sxs-lookup"><span data-stu-id="16f1e-440">With the **encoderType** parameter configured to use *AntiXssEncoder*, all output encoding within ASP.NET automatically uses the new encoding routines.</span></span>
8. <span data-ttu-id="16f1e-441">ASP.NET 4.5 请求验证还支持对请求数据的未经确认的访问。</span><span class="sxs-lookup"><span data-stu-id="16f1e-441">ASP.NET 4.5 request validation also supports un-validated access to request data.</span></span> <span data-ttu-id="16f1e-442">ASP.NET 4.5 中添加到新集合属性**HttpRequest**调用对象**Unvalidated**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-442">ASP.NET 4.5 adds a new collection property to the **HttpRequest** object called **Unvalidated**.</span></span> <span data-ttu-id="16f1e-443">导航到**HttpRequest.Unvalidated**有权访问所有请求数据，包括窗体、 QueryStrings、 Cookie、 Url 和等等的公共部分。</span><span class="sxs-lookup"><span data-stu-id="16f1e-443">When you navigate into **HttpRequest.Unvalidated** you have access to all of the common pieces of request data, including Forms, QueryStrings, Cookies, URLs, and so on.</span></span>

    <span data-ttu-id="16f1e-444">![Request.Unvalidated 对象](whats-new-in-web-forms-in-aspnet-45/_static/image22.png "Request.Unvalidated 对象")</span><span class="sxs-lookup"><span data-stu-id="16f1e-444">![Request.Unvalidated object](whats-new-in-web-forms-in-aspnet-45/_static/image22.png "Request.Unvalidated object")</span></span>

    <span data-ttu-id="16f1e-445">*Request.Unvalidated 对象*</span><span class="sxs-lookup"><span data-stu-id="16f1e-445">*Request.Unvalidated object*</span></span>

    > [!NOTE]
    > <span data-ttu-id="16f1e-446">**请谨慎使用 HttpRequest.Unvalidated 属性 ！**</span><span class="sxs-lookup"><span data-stu-id="16f1e-446">**Please use the HttpRequest.Unvalidated property with caution!**</span></span> <span data-ttu-id="16f1e-447">请确保仔细原始请求数据，以确保危险的文本是往返，并且不返回到信任客户呈现上执行自定义验证 ！</span><span class="sxs-lookup"><span data-stu-id="16f1e-447">Make sure you carefully perform custom validation on the raw request data to ensure that dangerous text is not round-tripped and rendered back to unsuspecting customers!</span></span>

<a id="Exercise3"></a>

<a id="Exercise_3_Asynchronous_Page_Processing_in_ASPNET_Web_Forms"></a>
### <a name="exercise-3-asynchronous-page-processing-in-aspnet-web-forms"></a><span data-ttu-id="16f1e-448">练习 3： 异步页处理在 ASP.NET Web 窗体</span><span class="sxs-lookup"><span data-stu-id="16f1e-448">Exercise 3: Asynchronous Page Processing in ASP.NET Web Forms</span></span>

<span data-ttu-id="16f1e-449">在此练习中，将向你介绍处理 ASP.NET Web 窗体中的功能的新异步页。</span><span class="sxs-lookup"><span data-stu-id="16f1e-449">In this exercise, you will be introduced to the new asynchronous page processing features in ASP.NET Web Forms.</span></span>

<a id="Task_1_-_Updating_the_Product_Details_Page_to_Upload_and_Show_Images"></a>
#### <a name="task-1---updating-the-product-details-page-to-upload-and-show-images"></a><span data-ttu-id="16f1e-450">任务 1-更新产品详细信息页面上载并显示图像</span><span class="sxs-lookup"><span data-stu-id="16f1e-450">Task 1 - Updating the Product Details Page to Upload and Show Images</span></span>

<span data-ttu-id="16f1e-451">在此任务中，你将更新产品详细信息页后，可以允许用户指定产品的图像 URL 并将其显示在只读视图。</span><span class="sxs-lookup"><span data-stu-id="16f1e-451">In this task, you will update the product details page to allow the user to specify an image URL for the product and display it in the read-only view.</span></span> <span data-ttu-id="16f1e-452">将通过同步下载创建指定的映像的本地副本。</span><span class="sxs-lookup"><span data-stu-id="16f1e-452">You will create a local copy of the specified image by downloading it synchronously.</span></span> <span data-ttu-id="16f1e-453">在下一步的任务中，你将更新此实现，使其以异步方式工作。</span><span class="sxs-lookup"><span data-stu-id="16f1e-453">In the next task, you will update this implementation to make it work asynchronously.</span></span>

1. <span data-ttu-id="16f1e-454">打开**Visual Studio 2012**和加载**开始**解决方案位于**Source\Ex3 Async\Begin**从本实验的文件夹。</span><span class="sxs-lookup"><span data-stu-id="16f1e-454">Open **Visual Studio 2012** and load the **Begin** solution located in **Source\Ex3-Async\Begin** from this lab's folder.</span></span> <span data-ttu-id="16f1e-455">或者，您可以对从前面的练习现有解决方案中继续工作。</span><span class="sxs-lookup"><span data-stu-id="16f1e-455">Alternatively, you can continue working on your existing solution from the previous exercises.</span></span>

   1. <span data-ttu-id="16f1e-456">如果你打开提供**开始**解决方案，你将需要下载一些缺少的 NuGet 程序包才能继续。</span><span class="sxs-lookup"><span data-stu-id="16f1e-456">If you opened the provided **Begin** solution, you will need to download some missing NuGet packages before continue.</span></span> <span data-ttu-id="16f1e-457">为此，请在解决方案资源管理器中，单击**WebFormsLab**项目，然后选择**管理 NuGet 包**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-457">To do this, in the Solution Explorer, click the **WebFormsLab** project and select **Manage NuGet Packages**.</span></span>
   2. <span data-ttu-id="16f1e-458">在**管理 NuGet 包**对话框中，单击**还原**以便下载缺少的程序包。</span><span class="sxs-lookup"><span data-stu-id="16f1e-458">In the **Manage NuGet Packages** dialog, click **Restore** in order to download missing packages.</span></span>
   3. <span data-ttu-id="16f1e-459">最后，通过单击生成解决方案**生成** | **生成解决方案**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-459">Finally, build the solution by clicking **Build** | **Build Solution**.</span></span>

      > [!NOTE]
      > <span data-ttu-id="16f1e-460">使用 NuGet 的优点之一是，你无需提供你的项目中的所有库减小项目大小。</span><span class="sxs-lookup"><span data-stu-id="16f1e-460">One of the advantages of using NuGet is that you don't have to ship all the libraries in your project, reducing the project size.</span></span> <span data-ttu-id="16f1e-461">使用 NuGet 增强工具，请通过指定的包版本在 Packages.config 文件中，你将能够下载首次运行该项目的所有所需的库。</span><span class="sxs-lookup"><span data-stu-id="16f1e-461">With NuGet Power Tools, by specifying the package versions in the Packages.config file, you will be able to download all the required libraries the first time you run the project.</span></span> <span data-ttu-id="16f1e-462">这是你将需要从本实验打开现有的解决方案后运行这些步骤的原因。</span><span class="sxs-lookup"><span data-stu-id="16f1e-462">This is why you will have to run these steps after you open an existing solution from this lab.</span></span>
2. <span data-ttu-id="16f1e-463">打开**ProductDetails.aspx**页上的源和 FormView ItemTemplate，以显示产品映像中添加字段。</span><span class="sxs-lookup"><span data-stu-id="16f1e-463">Open the **ProductDetails.aspx** page source and add a field in the FormView's ItemTemplate to show the product image.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample30.aspx)]
3. <span data-ttu-id="16f1e-464">添加字段以在 FormView EditTemplate 中指定图像 URL。</span><span class="sxs-lookup"><span data-stu-id="16f1e-464">Add a field to specify the image URL in the FormView's EditTemplate.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample31.aspx)]
4. <span data-ttu-id="16f1e-465">打开**ProductDetails.aspx.cs**代码隐藏文件，并添加以下命名空间指令。</span><span class="sxs-lookup"><span data-stu-id="16f1e-465">Open the **ProductDetails.aspx.cs** code-behind file and add the following namespace directives.</span></span>

    <span data-ttu-id="16f1e-466">(代码段- *Web 窗体实验室-Ex03-命名空间*)</span><span class="sxs-lookup"><span data-stu-id="16f1e-466">(Code Snippet - *Web Forms Lab - Ex03 - Namespaces*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample32.cs)]
5. <span data-ttu-id="16f1e-467">创建**UpdateProductImage**方法在本地中存储远程图像**映像**文件夹和更新产品实体与新的映像位置值。</span><span class="sxs-lookup"><span data-stu-id="16f1e-467">Create an **UpdateProductImage** method to store remote images in the local **Images** folder and update the product entity with the new image location value.</span></span>

    <span data-ttu-id="16f1e-468">(代码段- *Web 窗体实验室-Ex03-UpdateProductImage*)</span><span class="sxs-lookup"><span data-stu-id="16f1e-468">(Code Snippet - *Web Forms Lab - Ex03 - UpdateProductImage*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample33.cs)]
6. <span data-ttu-id="16f1e-469">更新**UpdateProduct**方法来调用**UpdateProductImage**方法。</span><span class="sxs-lookup"><span data-stu-id="16f1e-469">Update the **UpdateProduct** method to call the **UpdateProductImage** method.</span></span>

    <span data-ttu-id="16f1e-470">(代码段- *Web 窗体实验室-Ex03-UpdateProductImage 调用*)</span><span class="sxs-lookup"><span data-stu-id="16f1e-470">(Code Snippet - *Web Forms Lab - Ex03 - UpdateProductImage Call*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample34.cs)]
7. <span data-ttu-id="16f1e-471">运行应用程序并重试上载产品的映像。</span><span class="sxs-lookup"><span data-stu-id="16f1e-471">Run the application and try to upload an image for a product.</span></span> <span data-ttu-id="16f1e-472">例如，你可以使用从 Office 剪贴画以下图像 URL: [[http://officeimg.vo.msecnd.net/images/MB900437099.jpg](http://officeimg.vo.msecnd.net/images/MB900437099.jpg)](http://officeimg.vo.msecnd.net/images/MB900437099.jpg)</span><span class="sxs-lookup"><span data-stu-id="16f1e-472">For example, you can use the following image URL from Office Clip Arts: [[http://officeimg.vo.msecnd.net/images/MB900437099.jpg](http://officeimg.vo.msecnd.net/images/MB900437099.jpg)](http://officeimg.vo.msecnd.net/images/MB900437099.jpg)</span></span>

    <span data-ttu-id="16f1e-473">![设置产品的映像](whats-new-in-web-forms-in-aspnet-45/_static/image23.png "设置产品的映像")</span><span class="sxs-lookup"><span data-stu-id="16f1e-473">![Setting an image for a product](whats-new-in-web-forms-in-aspnet-45/_static/image23.png "Setting an image for a product")</span></span>

    <span data-ttu-id="16f1e-474">*设置产品的映像*</span><span class="sxs-lookup"><span data-stu-id="16f1e-474">*Setting an image for a product*</span></span>

<a id="Task_2_-_Adding_Asynchronous_Processing_to_the_Product_Details_Page"></a>
#### <a name="task-2---adding-asynchronous-processing-to-the-product-details-page"></a><span data-ttu-id="16f1e-475">任务 2-添加异步处理于产品详细信息页面</span><span class="sxs-lookup"><span data-stu-id="16f1e-475">Task 2 - Adding Asynchronous Processing to the Product Details Page</span></span>

<span data-ttu-id="16f1e-476">在此任务中，你将更新产品详细信息页，以使其以异步方式工作。</span><span class="sxs-lookup"><span data-stu-id="16f1e-476">In this task, you will update the product details page to make it work asynchronously.</span></span> <span data-ttu-id="16f1e-477">将通过使用 ASP.NET 4.5 异步页处理来提高长时间运行任务-映像下载过程的。</span><span class="sxs-lookup"><span data-stu-id="16f1e-477">You will enhance a long running task - the image download process - by using ASP.NET 4.5 asynchronous page processing.</span></span>

<span data-ttu-id="16f1e-478">Web 应用程序中的异步方法可以用于优化 ASP.NET 线程池的使用的方式。</span><span class="sxs-lookup"><span data-stu-id="16f1e-478">Asynchronous methods in web applications can be used to optimize the way ASP.NET thread pools are used.</span></span> <span data-ttu-id="16f1e-479">在 ASP.NET 中存在是有限的数量的越过线程池中的线程请求，因此，在所有线程都非常忙，ASP.NET 开始拒绝新请求，发送应用程序错误消息并使你的站点不可用。</span><span class="sxs-lookup"><span data-stu-id="16f1e-479">In ASP.NET there are a limited number of threads in the thread pool for attending requests, thus, when all the threads are busy, ASP.NET starts to reject new requests, sends application error messages and makes your site unavailable.</span></span>

<span data-ttu-id="16f1e-480">在网站上耗时的操作都适合进行异步编程，因为它们将长时间占用分配的线程。</span><span class="sxs-lookup"><span data-stu-id="16f1e-480">Time-consuming operations on your web site are great candidates for asynchronous programming because they occupy the assigned thread for a long time.</span></span> <span data-ttu-id="16f1e-481">这包括长时间运行的请求，具有许多不同的元素的页和需要脱机操作中，将此类查询数据库或访问的外部 web 服务器的页。</span><span class="sxs-lookup"><span data-stu-id="16f1e-481">This includes long running requests, pages with lots of different elements and pages that require offline operations, such querying a database or accessing an external web server.</span></span> <span data-ttu-id="16f1e-482">优点是处理页时，你会为这些操作，使用异步方法，如果线程是释放并返回到的线程池，以便用于处理新的页面请求。</span><span class="sxs-lookup"><span data-stu-id="16f1e-482">The advantage is that if you use asynchronous methods for these operations, while the page is processing, the thread is freed and returned to the thread pool and can be used to attend to a new page request.</span></span> <span data-ttu-id="16f1e-483">这意味着，该页面才会开始在线程池中的一个线程中处理，并可能在异步处理完成后完成在一个不同的处理。</span><span class="sxs-lookup"><span data-stu-id="16f1e-483">This means, the page will start processing in one thread from the thread pool and might complete processing in a different one, after the async processing completes.</span></span>

1. <span data-ttu-id="16f1e-484">打开**ProductDetails.aspx**页。</span><span class="sxs-lookup"><span data-stu-id="16f1e-484">Open the **ProductDetails.aspx** page.</span></span> <span data-ttu-id="16f1e-485">添加**异步**属性中**页**元素并将其设置为**true**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-485">Add the **Async** attribute in the **Page** element and set it to **true**.</span></span> <span data-ttu-id="16f1e-486">此属性是告诉 ASP.NET 实现 IHttpAsyncHandler 接口。</span><span class="sxs-lookup"><span data-stu-id="16f1e-486">This attribute tells ASP.NET to implement the IHttpAsyncHandler interface.</span></span>


~~~
[!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample35.aspx)]
~~~
2. <span data-ttu-id="16f1e-487">在要显示的线程运行页面的详细信息的页的底部添加标签。</span><span class="sxs-lookup"><span data-stu-id="16f1e-487">Add a Label at the bottom of the page to show the details of the threads running the page.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample36.aspx)]
3. <span data-ttu-id="16f1e-488">打开**ProductDetails.aspx.cs**并添加以下命名空间指令。</span><span class="sxs-lookup"><span data-stu-id="16f1e-488">Open up **ProductDetails.aspx.cs** and add the following namespace directives.</span></span>

    <span data-ttu-id="16f1e-489">(代码段- *Web 窗体实验室-Ex03-命名空间 2*)</span><span class="sxs-lookup"><span data-stu-id="16f1e-489">(Code Snippet - *Web Forms Lab - Ex03 - Namespaces 2*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample37.cs)]
4. <span data-ttu-id="16f1e-490">修改**UpdateProductImage**方法与异步任务将映像下载。</span><span class="sxs-lookup"><span data-stu-id="16f1e-490">Modify the **UpdateProductImage** method to download the image with an asynchronous task.</span></span> <span data-ttu-id="16f1e-491">将替换**WebClient** **后**方法替换**DownloadFileTaskAsync**方法和包括**await**关键字。</span><span class="sxs-lookup"><span data-stu-id="16f1e-491">You will replace the **WebClient** **DownloadFile** method with the **DownloadFileTaskAsync** method and include the **await** keyword.</span></span>

    <span data-ttu-id="16f1e-492">(代码段- *Web 窗体实验室-Ex03-UpdateProductImage 异步*)</span><span class="sxs-lookup"><span data-stu-id="16f1e-492">(Code Snippet - *Web Forms Lab - Ex03 - UpdateProductImage Async*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample38.cs)]

    <span data-ttu-id="16f1e-493">RegisterAsyncTask 注册新的页异步任务要在不同线程中执行。</span><span class="sxs-lookup"><span data-stu-id="16f1e-493">The RegisterAsyncTask registers a new page asynchronous task to be executed in a different thread.</span></span> <span data-ttu-id="16f1e-494">它会收到要执行的任务 (t) 的 lambda 表达式。</span><span class="sxs-lookup"><span data-stu-id="16f1e-494">It receives a lambda expression with the Task (t) to be executed.</span></span> <span data-ttu-id="16f1e-495">**Await**中的关键字**DownloadFileTaskAsync**方法将转换后，将异步调用的回调方法的其余部分**DownloadFileTaskAsync**方法已完成。</span><span class="sxs-lookup"><span data-stu-id="16f1e-495">The **await** keyword in the **DownloadFileTaskAsync** method converts the remainder of the method into a callback that is invoked asynchronously after the **DownloadFileTaskAsync** method has completed.</span></span> <span data-ttu-id="16f1e-496">ASP.NET 将通过自动维护所有原始 HTTP 请求值来继续执行该方法。</span><span class="sxs-lookup"><span data-stu-id="16f1e-496">ASP.NET will resume the execution of the method by automatically maintaining all the HTTP request original values.</span></span> <span data-ttu-id="16f1e-497">.NET 4.5 中的新异步编程模型，可编写异步代码，看起来非常相似的同步代码，并让编译器处理回调函数或延续代码的复杂性。</span><span class="sxs-lookup"><span data-stu-id="16f1e-497">The new asynchronous programming model in .NET 4.5 enables you to write asynchronous code that looks very much like synchronous code, and let the compiler handle the complications of callback functions or continuation code.</span></span>
    > [!NOTE]
    > <span data-ttu-id="16f1e-498">RegisterAsyncTask 和 PageAsyncTask 自.NET 2.0 以来已可供。</span><span class="sxs-lookup"><span data-stu-id="16f1e-498">RegisterAsyncTask and PageAsyncTask were already available since .NET 2.0.</span></span> <span data-ttu-id="16f1e-499">Await 关键字为新从.NET 4.5 的异步编程模型，并可与从.NET WebClient 对象的新 TaskAsync 方法结合使用。</span><span class="sxs-lookup"><span data-stu-id="16f1e-499">The await keyword is new from the .NET 4.5 asynchronous programming model and can be used together with the new TaskAsync methods from the .NET WebClient object.</span></span>
5. <span data-ttu-id="16f1e-500">添加代码以显示的线程的代码在其启动和完成执行。</span><span class="sxs-lookup"><span data-stu-id="16f1e-500">Add code to display the threads on which the code started and finished executing.</span></span> <span data-ttu-id="16f1e-501">若要执行此操作，更新**UpdateProductImage**方法替换为以下代码。</span><span class="sxs-lookup"><span data-stu-id="16f1e-501">To do this, update the **UpdateProductImage** method with the following code.</span></span>

    <span data-ttu-id="16f1e-502">(代码段- *Web 窗体实验室-Ex03-显示线程*)</span><span class="sxs-lookup"><span data-stu-id="16f1e-502">(Code Snippet - *Web Forms Lab - Ex03 - Show threads*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample39.cs)]
6. <span data-ttu-id="16f1e-503">打开网站的**Web.config**文件。</span><span class="sxs-lookup"><span data-stu-id="16f1e-503">Open the web site's **Web.config** file.</span></span> <span data-ttu-id="16f1e-504">添加以下 appSetting 变量。</span><span class="sxs-lookup"><span data-stu-id="16f1e-504">Add the following appSetting variable.</span></span>

    [!code-xml[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample40.xml)]
7. <span data-ttu-id="16f1e-505">按**F5**运行应用程序并上载该产品的映像。</span><span class="sxs-lookup"><span data-stu-id="16f1e-505">Press **F5** to run the application and upload an image for the product.</span></span> <span data-ttu-id="16f1e-506">请注意开始和完成的代码可能不同的线程 ID。</span><span class="sxs-lookup"><span data-stu-id="16f1e-506">Notice the threads ID where the code started and finished may be different.</span></span> <span data-ttu-id="16f1e-507">这是因为从 ASP.NET 线程池的单独线程上运行异步任务。</span><span class="sxs-lookup"><span data-stu-id="16f1e-507">This is because asynchronous tasks run on a separate thread from ASP.NET thread pool.</span></span> <span data-ttu-id="16f1e-508">在任务完成后，ASP.NET 将任务放回队列中，并将分配任何可用线程。</span><span class="sxs-lookup"><span data-stu-id="16f1e-508">When the task completes, ASP.NET puts the task back in the queue and assigns any of the available threads.</span></span>

    <span data-ttu-id="16f1e-509">![异步下载映像](whats-new-in-web-forms-in-aspnet-45/_static/image24.png "异步下载图像")</span><span class="sxs-lookup"><span data-stu-id="16f1e-509">![Downloading an image asynchronously](whats-new-in-web-forms-in-aspnet-45/_static/image24.png "Downloading an image asynchronously")</span></span>

    <span data-ttu-id="16f1e-510">*异步下载图像*</span><span class="sxs-lookup"><span data-stu-id="16f1e-510">*Downloading an image asynchronously*</span></span>

> [!NOTE]
> <span data-ttu-id="16f1e-511">此外，你可以部署此应用程序对 Azure 以下[附录 b： 发布 ASP.NET MVC 4 应用程序使用 Web Deploy](#AppendixB)。</span><span class="sxs-lookup"><span data-stu-id="16f1e-511">Additionally, you can deploy this application to Azure following [Appendix B: Publishing an ASP.NET MVC 4 Application using Web Deploy](#AppendixB).</span></span>


* * *

<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="16f1e-512">总结</span><span class="sxs-lookup"><span data-stu-id="16f1e-512">Summary</span></span>

<span data-ttu-id="16f1e-513">在此动手实验中，具有已发送并演示以下概念：</span><span class="sxs-lookup"><span data-stu-id="16f1e-513">In this hands-on lab, the following concepts have been addressed and demonstrated:</span></span>

- <span data-ttu-id="16f1e-514">使用强类型化数据绑定表达式</span><span class="sxs-lookup"><span data-stu-id="16f1e-514">Use strongly-typed data-binding expressions</span></span>
- <span data-ttu-id="16f1e-515">使用 Web 窗体中的新模型绑定功能</span><span class="sxs-lookup"><span data-stu-id="16f1e-515">Use new model binding features in Web Forms</span></span>
- <span data-ttu-id="16f1e-516">用于值在提供程序将页面数据映射到代码隐藏方法</span><span class="sxs-lookup"><span data-stu-id="16f1e-516">Use value providers for mapping page data to code-behind methods</span></span>
- <span data-ttu-id="16f1e-517">对于用户输入验证中使用数据注释</span><span class="sxs-lookup"><span data-stu-id="16f1e-517">Use Data Annotations for user input validation</span></span>
- <span data-ttu-id="16f1e-518">在 Web 窗体中执行 jQuery unobstrusive 客户端验证的 advange</span><span class="sxs-lookup"><span data-stu-id="16f1e-518">Take advange of unobstrusive client-side validation with jQuery in Web Forms</span></span>
- <span data-ttu-id="16f1e-519">实现精细请求验证</span><span class="sxs-lookup"><span data-stu-id="16f1e-519">Implement granular request validation</span></span>
- <span data-ttu-id="16f1e-520">实现异步处理在 Web 窗体中的页</span><span class="sxs-lookup"><span data-stu-id="16f1e-520">Implement asynchronous page processing in Web Forms</span></span>

<a id="AppendixA"></a>

<a id="Appendix_A_Installing_Visual_Studio_Express_2012_for_Web"></a>
## <a name="appendix-a-installing-visual-studio-express-2012-for-web"></a><span data-ttu-id="16f1e-521">附录 a： 安装 Visual Studio Express 2012 for Web</span><span class="sxs-lookup"><span data-stu-id="16f1e-521">Appendix A: Installing Visual Studio Express 2012 for Web</span></span>

<span data-ttu-id="16f1e-522">你可以安装**Microsoft Visual Studio Express 2012 for Web**或另一个&quot;Express&quot;版本使用 **[Microsoft Web 平台安装程序](https://www.microsoft.com/web/downloads/platform.aspx)**.</span><span class="sxs-lookup"><span data-stu-id="16f1e-522">You can install **Microsoft Visual Studio Express 2012 for Web** or another &quot;Express&quot; version using the **[Microsoft Web Platform Installer](https://www.microsoft.com/web/downloads/platform.aspx)**.</span></span> <span data-ttu-id="16f1e-523">以下说明将指导你完成安装所需的步骤*Visual studio Express 2012 for Web*使用*Microsoft Web 平台安装程序*。</span><span class="sxs-lookup"><span data-stu-id="16f1e-523">The following instructions guide you through the steps required to install *Visual studio Express 2012 for Web* using *Microsoft Web Platform Installer*.</span></span>

1. <span data-ttu-id="16f1e-524">转到[ [ https://go.microsoft.com/?linkid=9810169 ](https://go.microsoft.com/?linkid=9810169) ](https://go.microsoft.com/?linkid=9810169)。</span><span class="sxs-lookup"><span data-stu-id="16f1e-524">Go to [[https://go.microsoft.com/?linkid=9810169](https://go.microsoft.com/?linkid=9810169)](https://go.microsoft.com/?linkid=9810169).</span></span> <span data-ttu-id="16f1e-525">或者，如果你已安装 Web 平台安装程序，你可以打开它，并搜索产品&quot; <em>Visual Studio Express 2012 for Web 与 Azure SDK</em>&quot;。</span><span class="sxs-lookup"><span data-stu-id="16f1e-525">Alternatively, if you already have installed Web Platform Installer, you can open it and search for the product &quot;<em>Visual Studio Express 2012 for Web with Azure SDK</em>&quot;.</span></span>
2. <span data-ttu-id="16f1e-526">单击**立即安装**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-526">Click on **Install Now**.</span></span> <span data-ttu-id="16f1e-527">如果你没有**Web 平台安装程序**将重定向以下载并请先安装它。</span><span class="sxs-lookup"><span data-stu-id="16f1e-527">If you do not have **Web Platform Installer** you will be redirected to download and install it first.</span></span>
3. <span data-ttu-id="16f1e-528">一次**Web 平台安装程序**处于打开状态，单击**安装**以启动安装程序。</span><span class="sxs-lookup"><span data-stu-id="16f1e-528">Once **Web Platform Installer** is open, click **Install** to start the setup.</span></span>

    <span data-ttu-id="16f1e-529">![安装 Visual Studio Express](whats-new-in-web-forms-in-aspnet-45/_static/image25.png "安装 Visual Studio Express")</span><span class="sxs-lookup"><span data-stu-id="16f1e-529">![Install Visual Studio Express](whats-new-in-web-forms-in-aspnet-45/_static/image25.png "Install Visual Studio Express")</span></span>

    <span data-ttu-id="16f1e-530">*安装 Visual Studio Express*</span><span class="sxs-lookup"><span data-stu-id="16f1e-530">*Install Visual Studio Express*</span></span>
4. <span data-ttu-id="16f1e-531">阅读所有产品的许可证和条款，然后单击**我接受**以继续。</span><span class="sxs-lookup"><span data-stu-id="16f1e-531">Read all the products' licenses and terms and click **I Accept** to continue.</span></span>

    ![接受许可条款](whats-new-in-web-forms-in-aspnet-45/_static/image26.png)

    <span data-ttu-id="16f1e-533">*接受许可条款*</span><span class="sxs-lookup"><span data-stu-id="16f1e-533">*Accepting the license terms*</span></span>
5. <span data-ttu-id="16f1e-534">等待，直到下载和安装过程完成。</span><span class="sxs-lookup"><span data-stu-id="16f1e-534">Wait until the downloading and installation process completes.</span></span>

    ![安装进度](whats-new-in-web-forms-in-aspnet-45/_static/image27.png)

    <span data-ttu-id="16f1e-536">*安装进度*</span><span class="sxs-lookup"><span data-stu-id="16f1e-536">*Installation progress*</span></span>
6. <span data-ttu-id="16f1e-537">当安装完成后时，单击**完成**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-537">When the installation completes, click **Finish**.</span></span>

    ![安装已完成](whats-new-in-web-forms-in-aspnet-45/_static/image28.png)

    <span data-ttu-id="16f1e-539">*安装已完成*</span><span class="sxs-lookup"><span data-stu-id="16f1e-539">*Installation completed*</span></span>
7. <span data-ttu-id="16f1e-540">单击**退出**以关闭 Web 平台安装程序。</span><span class="sxs-lookup"><span data-stu-id="16f1e-540">Click **Exit** to close Web Platform Installer.</span></span>
8. <span data-ttu-id="16f1e-541">若要打开 Visual Studio Express for Web，请转到**启动**屏幕并开始编写&quot; **VS Express**&quot;，然后单击**VS Express for Web**磁贴。</span><span class="sxs-lookup"><span data-stu-id="16f1e-541">To open Visual Studio Express for Web, go to the **Start** screen and start writing &quot;**VS Express**&quot;, then click on the **VS Express for Web** tile.</span></span>

    ![Web 磁贴的 VS Express](whats-new-in-web-forms-in-aspnet-45/_static/image29.png)

    <span data-ttu-id="16f1e-543">*Web 磁贴的 VS Express*</span><span class="sxs-lookup"><span data-stu-id="16f1e-543">*VS Express for Web tile*</span></span>

<a id="AppendixB"></a>

<a id="Appendix_B_Publishing_an_ASPNET_MVC_4_Application_using_Web_Deploy"></a>
## <a name="appendix-b-publishing-an-aspnet-mvc-4-application-using-web-deploy"></a><span data-ttu-id="16f1e-544">附录 b： 发布 ASP.NET MVC 4 应用程序使用 Web 部署</span><span class="sxs-lookup"><span data-stu-id="16f1e-544">Appendix B: Publishing an ASP.NET MVC 4 Application using Web Deploy</span></span>

<span data-ttu-id="16f1e-545">本附录将演示如何从 Azure 门户创建新的网站和发布应用程序获取按照本实验中，利用 Azure 提供的 Web 部署发布功能。</span><span class="sxs-lookup"><span data-stu-id="16f1e-545">This appendix will show you how to create a new web site from the Azure Portal and publish the application you obtained by following the lab, taking advantage of the Web Deploy publishing feature provided by Azure.</span></span>

<a id="ApxBTask1"></a>

<a id="Task_1_-_Creating_a_New_Web_Site_from_the_Windows_Azure_Portal"></a>
#### <a name="task-1---creating-a-new-web-site-from-the-azure-portal"></a><span data-ttu-id="16f1e-546">任务 1-从 Azure 门户创建新网站</span><span class="sxs-lookup"><span data-stu-id="16f1e-546">Task 1 - Creating a New Web Site from the Azure Portal</span></span>

1. <span data-ttu-id="16f1e-547">转到[Azure 管理门户](https://manage.windowsazure.com/)并使用与你的订阅关联的 Microsoft 凭据登录。</span><span class="sxs-lookup"><span data-stu-id="16f1e-547">Go to the [Azure Management Portal](https://manage.windowsazure.com/) and sign in using the Microsoft credentials associated with your subscription.</span></span>

    > [!NOTE]
    > <span data-ttu-id="16f1e-548">使用 Azure 可以免费承载 10 个 ASP.NET 网站，然后随着流量增长而扩展。</span><span class="sxs-lookup"><span data-stu-id="16f1e-548">With Azure you can host 10 ASP.NET Web Sites for free and then scale as your traffic grows.</span></span> <span data-ttu-id="16f1e-549">你可以注册[此处](http://aka.ms/aspnet-hol-azure)。</span><span class="sxs-lookup"><span data-stu-id="16f1e-549">You can sign up [here](http://aka.ms/aspnet-hol-azure).</span></span>

    <span data-ttu-id="16f1e-550">![登录到 Windows Azure 门户](whats-new-in-web-forms-in-aspnet-45/_static/image30.png "登录到 Windows Azure 门户")</span><span class="sxs-lookup"><span data-stu-id="16f1e-550">![Log on to Windows Azure portal](whats-new-in-web-forms-in-aspnet-45/_static/image30.png "Log on to Windows Azure portal")</span></span>

    <span data-ttu-id="16f1e-551">*登录到门户*</span><span class="sxs-lookup"><span data-stu-id="16f1e-551">*Log on to the Portal*</span></span>
2. <span data-ttu-id="16f1e-552">单击**新建**命令栏上。</span><span class="sxs-lookup"><span data-stu-id="16f1e-552">Click **New** on the command bar.</span></span>

    <span data-ttu-id="16f1e-553">![创建新网站](whats-new-in-web-forms-in-aspnet-45/_static/image31.png "创建新网站")</span><span class="sxs-lookup"><span data-stu-id="16f1e-553">![Creating a new Web Site](whats-new-in-web-forms-in-aspnet-45/_static/image31.png "Creating a new Web Site")</span></span>

    <span data-ttu-id="16f1e-554">*创建新网站*</span><span class="sxs-lookup"><span data-stu-id="16f1e-554">*Creating a new Web Site*</span></span>
3. <span data-ttu-id="16f1e-555">单击**计算** | **网站**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-555">Click **Compute** | **Web Site**.</span></span> <span data-ttu-id="16f1e-556">然后选择**快速创建**选项。</span><span class="sxs-lookup"><span data-stu-id="16f1e-556">Then select **Quick Create** option.</span></span> <span data-ttu-id="16f1e-557">为新网站提供可用的 URL，然后单击**创建网站**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-557">Provide an available URL for the new web site and click **Create Web Site**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="16f1e-558">Azure 是运行在云中，你可以控制和管理 web 应用程序的宿主。</span><span class="sxs-lookup"><span data-stu-id="16f1e-558">Azure is the host for a web application running in the cloud that you can control and manage.</span></span> <span data-ttu-id="16f1e-559">快速创建选项，可部署到从 Azure 门户外部的已完成的 web 应用程序。</span><span class="sxs-lookup"><span data-stu-id="16f1e-559">The Quick Create option allows you to deploy a completed web application to the Azure from outside the portal.</span></span> <span data-ttu-id="16f1e-560">它不包括用于设置数据库的步骤。</span><span class="sxs-lookup"><span data-stu-id="16f1e-560">It does not include steps for setting up a database.</span></span>

    <span data-ttu-id="16f1e-561">![创建新的网站使用快速创建](whats-new-in-web-forms-in-aspnet-45/_static/image32.png "创建新的网站使用快速创建")</span><span class="sxs-lookup"><span data-stu-id="16f1e-561">![Creating a new Web Site using Quick Create](whats-new-in-web-forms-in-aspnet-45/_static/image32.png "Creating a new Web Site using Quick Create")</span></span>

    <span data-ttu-id="16f1e-562">*创建新的网站使用快速创建*</span><span class="sxs-lookup"><span data-stu-id="16f1e-562">*Creating a new Web Site using Quick Create*</span></span>
4. <span data-ttu-id="16f1e-563">等到新**网站**创建。</span><span class="sxs-lookup"><span data-stu-id="16f1e-563">Wait until the new **Web Site** is created.</span></span>
5. <span data-ttu-id="16f1e-564">创建网站后单击下的链接**URL**列。</span><span class="sxs-lookup"><span data-stu-id="16f1e-564">Once the Web Site is created click the link under the **URL** column.</span></span> <span data-ttu-id="16f1e-565">检查新的 Web 站点工作。</span><span class="sxs-lookup"><span data-stu-id="16f1e-565">Check that the new Web Site is working.</span></span>

    <span data-ttu-id="16f1e-566">![浏览到新的 web 站点](whats-new-in-web-forms-in-aspnet-45/_static/image33.png "浏览到新的 web 站点")</span><span class="sxs-lookup"><span data-stu-id="16f1e-566">![Browsing to the new web site](whats-new-in-web-forms-in-aspnet-45/_static/image33.png "Browsing to the new web site")</span></span>

    <span data-ttu-id="16f1e-567">*浏览到新的 web 站点*</span><span class="sxs-lookup"><span data-stu-id="16f1e-567">*Browsing to the new web site*</span></span>

    <span data-ttu-id="16f1e-568">![运行网站](whats-new-in-web-forms-in-aspnet-45/_static/image34.png "运行的网站")</span><span class="sxs-lookup"><span data-stu-id="16f1e-568">![Web site running](whats-new-in-web-forms-in-aspnet-45/_static/image34.png "Web site running")</span></span>

    <span data-ttu-id="16f1e-569">*运行的网站*</span><span class="sxs-lookup"><span data-stu-id="16f1e-569">*Web site running*</span></span>
6. <span data-ttu-id="16f1e-570">返回到门户并单击在网站的名称**名称**列以显示的管理页。</span><span class="sxs-lookup"><span data-stu-id="16f1e-570">Go back to the portal and click the name of the web site under the **Name** column to display the management pages.</span></span>

    <span data-ttu-id="16f1e-571">![打开网站管理页](whats-new-in-web-forms-in-aspnet-45/_static/image35.png "打开网站管理页")</span><span class="sxs-lookup"><span data-stu-id="16f1e-571">![Opening the web site management pages](whats-new-in-web-forms-in-aspnet-45/_static/image35.png "Opening the web site management pages")</span></span>

    <span data-ttu-id="16f1e-572">*打开网站管理页*</span><span class="sxs-lookup"><span data-stu-id="16f1e-572">*Opening the Web Site management pages*</span></span>
7. <span data-ttu-id="16f1e-573">在**仪表板**页上，在**速览**部分中，单击**下载发布配置文件**链接。</span><span class="sxs-lookup"><span data-stu-id="16f1e-573">In the **Dashboard** page, under the **quick glance** section, click the **Download publish profile** link.</span></span>

    > [!NOTE]
    > <span data-ttu-id="16f1e-574">*发布配置文件*包含所有发布到 Azure 的 web 应用程序的每个启用的发布方法所需的信息。</span><span class="sxs-lookup"><span data-stu-id="16f1e-574">The *publish profile* contains all of the information required to publish a web application to Azure for each enabled publication method.</span></span> <span data-ttu-id="16f1e-575">发布配置文件包含的 Url、 用户凭据和连接到并针对每个发布方法启用的终结点进行身份验证所需的数据库字符串。</span><span class="sxs-lookup"><span data-stu-id="16f1e-575">The publish profile contains the URLs, user credentials and database strings required to connect to and authenticate against each of the endpoints for which a publication method is enabled.</span></span> <span data-ttu-id="16f1e-576">**Microsoft WebMatrix 2**， **Microsoft Visual Studio Express for Web**和**Microsoft Visual Studio 2012**支持读取发布配置文件以自动执行的这些程序，以便配置web 应用程序发布到 Azure。</span><span class="sxs-lookup"><span data-stu-id="16f1e-576">**Microsoft WebMatrix 2**, **Microsoft Visual Studio Express for Web** and **Microsoft Visual Studio 2012** support reading publish profiles to automate configuration of these programs for publishing web applications to Azure.</span></span>

    <span data-ttu-id="16f1e-577">![下载网站发布配置文件](whats-new-in-web-forms-in-aspnet-45/_static/image36.png "下载网站发布配置文件")</span><span class="sxs-lookup"><span data-stu-id="16f1e-577">![Downloading the web site publish profile](whats-new-in-web-forms-in-aspnet-45/_static/image36.png "Downloading the web site publish profile")</span></span>

    <span data-ttu-id="16f1e-578">*下载网站发布配置文件*</span><span class="sxs-lookup"><span data-stu-id="16f1e-578">*Downloading the Web Site publish profile*</span></span>
8. <span data-ttu-id="16f1e-579">到一个已知位置下载发布配置文件。</span><span class="sxs-lookup"><span data-stu-id="16f1e-579">Download the publish profile file to a known location.</span></span> <span data-ttu-id="16f1e-580">进一步在本练习中，你将了解如何使用此文件发布到 Azure web 应用程序从 Visual Studio。</span><span class="sxs-lookup"><span data-stu-id="16f1e-580">Further in this exercise you will see how to use this file to publish a web application to Azure from Visual Studio.</span></span>

    <span data-ttu-id="16f1e-581">![保存发布配置文件](whats-new-in-web-forms-in-aspnet-45/_static/image37.png "保存发布配置文件")</span><span class="sxs-lookup"><span data-stu-id="16f1e-581">![Saving the publish profile file](whats-new-in-web-forms-in-aspnet-45/_static/image37.png "Saving the publish profile")</span></span>

    <span data-ttu-id="16f1e-582">*保存发布配置文件*</span><span class="sxs-lookup"><span data-stu-id="16f1e-582">*Saving the publish profile file*</span></span>

<a id="ApxBTask2"></a>

<a id="Task_2_-_Configuring_the_Database_Server"></a>
#### <a name="task-2---configuring-the-database-server"></a><span data-ttu-id="16f1e-583">任务 2-配置数据库服务器</span><span class="sxs-lookup"><span data-stu-id="16f1e-583">Task 2 - Configuring the Database Server</span></span>

<span data-ttu-id="16f1e-584">如果你的应用程序将使用 SQL Server 数据库将需要创建 SQL 数据库服务器。</span><span class="sxs-lookup"><span data-stu-id="16f1e-584">If your application makes use of SQL Server databases you will need to create a SQL Database server.</span></span> <span data-ttu-id="16f1e-585">如果你想要部署的简单应用程序不使用 SQL Server 可能会跳过此任务。</span><span class="sxs-lookup"><span data-stu-id="16f1e-585">If you want to deploy a simple application that does not use SQL Server you might skip this task.</span></span>

1. <span data-ttu-id="16f1e-586">需要将用于存储应用程序数据库的 SQL 数据库服务器。</span><span class="sxs-lookup"><span data-stu-id="16f1e-586">You will need a SQL Database server for storing the application database.</span></span> <span data-ttu-id="16f1e-587">你可以从你的订阅在 Azure 管理门户中查看 SQL 数据库服务器**Sql 数据库** | **服务器** | **服务器的仪表板**.</span><span class="sxs-lookup"><span data-stu-id="16f1e-587">You can view the SQL Database servers from your subscription in the Azure Management portal at **Sql Databases** | **Servers** | **Server's Dashboard**.</span></span> <span data-ttu-id="16f1e-588">如果你没有创建的服务器，则可以创建一个使用**添加**命令栏上的按钮。</span><span class="sxs-lookup"><span data-stu-id="16f1e-588">If you do not have a server created, you can create one using the **Add** button on the command bar.</span></span> <span data-ttu-id="16f1e-589">请记下的**服务器名称和 URL、 管理员登录名和密码**，如你将在接下来的任务中使用它们。</span><span class="sxs-lookup"><span data-stu-id="16f1e-589">Take note of the **server name and URL, administrator login name and password**, as you will use them in the next tasks.</span></span> <span data-ttu-id="16f1e-590">不创建数据库，它将在后面的阶段中创建。</span><span class="sxs-lookup"><span data-stu-id="16f1e-590">Do not create the database yet, as it will be created in a later stage.</span></span>

    <span data-ttu-id="16f1e-591">![SQL 数据库服务器仪表板](whats-new-in-web-forms-in-aspnet-45/_static/image38.png "SQL Database 服务器仪表板")</span><span class="sxs-lookup"><span data-stu-id="16f1e-591">![SQL Database Server Dashboard](whats-new-in-web-forms-in-aspnet-45/_static/image38.png "SQL Database Server Dashboard")</span></span>

    <span data-ttu-id="16f1e-592">*SQL 数据库服务器仪表板*</span><span class="sxs-lookup"><span data-stu-id="16f1e-592">*SQL Database Server Dashboard*</span></span>
2. <span data-ttu-id="16f1e-593">在下一个任务将测试从 Visual Studio 中，数据库连接，因此你需要在的服务器的列表中包括你的本地 IP 地址**允许的 IP 地址**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-593">In the next task you will test the database connection from Visual Studio, for that reason you need to include your local IP address in the server's list of **Allowed IP Addresses**.</span></span> <span data-ttu-id="16f1e-594">若要做到这一点，请单击**配置**，选择的 IP 地址从**当前客户端 IP 地址**并将其粘贴在**起始 IP 地址**和**结束 IP 地址**文本框和单击![add-client-ip-address-ok-button](whats-new-in-web-forms-in-aspnet-45/_static/image39.png)按钮。</span><span class="sxs-lookup"><span data-stu-id="16f1e-594">To do that, click **Configure**, select the IP address from **Current Client IP Address** and paste it on the **Start IP Address** and **End IP Address** text boxes and click the ![add-client-ip-address-ok-button](whats-new-in-web-forms-in-aspnet-45/_static/image39.png) button.</span></span>

    ![添加客户端 IP 地址](whats-new-in-web-forms-in-aspnet-45/_static/image40.png)

    <span data-ttu-id="16f1e-596">*添加客户端 IP 地址*</span><span class="sxs-lookup"><span data-stu-id="16f1e-596">*Adding Client IP Address*</span></span>
3. <span data-ttu-id="16f1e-597">一次**客户端 IP 地址**添加到允许的 IP 地址列表中，单击**保存**以确认所做的更改。</span><span class="sxs-lookup"><span data-stu-id="16f1e-597">Once the **Client IP Address** is added to the allowed IP addresses list, click on **Save** to confirm the changes.</span></span>

    ![确认做的更改](whats-new-in-web-forms-in-aspnet-45/_static/image41.png)

    <span data-ttu-id="16f1e-599">*确认做的更改*</span><span class="sxs-lookup"><span data-stu-id="16f1e-599">*Confirm Changes*</span></span>

<a id="ApxBTask3"></a>

<a id="Task_3_-_Publishing_an_ASPNET_MVC_4_Application_using_Web_Deploy"></a>
#### <a name="task-3---publishing-an-aspnet-mvc-4-application-using-web-deploy"></a><span data-ttu-id="16f1e-600">任务 3-发布 ASP.NET MVC 4 应用程序使用 Web 部署</span><span class="sxs-lookup"><span data-stu-id="16f1e-600">Task 3 - Publishing an ASP.NET MVC 4 Application using Web Deploy</span></span>

1. <span data-ttu-id="16f1e-601">返回到 ASP.NET MVC 4 解决方案。</span><span class="sxs-lookup"><span data-stu-id="16f1e-601">Go back to the ASP.NET MVC 4 solution.</span></span> <span data-ttu-id="16f1e-602">在**解决方案资源管理器**，右键单击网站项目，然后选择**发布**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-602">In the **Solution Explorer**, right-click the web site project and select **Publish**.</span></span>

    <span data-ttu-id="16f1e-603">![发布应用程序](whats-new-in-web-forms-in-aspnet-45/_static/image42.png "发布应用程序")</span><span class="sxs-lookup"><span data-stu-id="16f1e-603">![Publishing the Application](whats-new-in-web-forms-in-aspnet-45/_static/image42.png "Publishing the Application")</span></span>

    <span data-ttu-id="16f1e-604">*发布此网站*</span><span class="sxs-lookup"><span data-stu-id="16f1e-604">*Publishing the web site*</span></span>
2. <span data-ttu-id="16f1e-605">导入发布配置文件保存在第一个任务。</span><span class="sxs-lookup"><span data-stu-id="16f1e-605">Import the publish profile you saved in the first task.</span></span>

    <span data-ttu-id="16f1e-606">![导入发布配置文件](whats-new-in-web-forms-in-aspnet-45/_static/image43.png "导入发布配置文件")</span><span class="sxs-lookup"><span data-stu-id="16f1e-606">![Importing the publish profile](whats-new-in-web-forms-in-aspnet-45/_static/image43.png "Importing the publish profile")</span></span>

    <span data-ttu-id="16f1e-607">*导入发布配置文件*</span><span class="sxs-lookup"><span data-stu-id="16f1e-607">*Importing publish profile*</span></span>
3. <span data-ttu-id="16f1e-608">单击**验证连接**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-608">Click **Validate Connection**.</span></span> <span data-ttu-id="16f1e-609">验证完成后单击**下一步**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-609">Once Validation is complete click **Next**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="16f1e-610">请参阅验证连接按钮旁边将显示绿色的复选标记后，验证已完成。</span><span class="sxs-lookup"><span data-stu-id="16f1e-610">Validation is complete once you see a green checkmark appear next to the Validate Connection button.</span></span>

    <span data-ttu-id="16f1e-611">![正在验证连接](whats-new-in-web-forms-in-aspnet-45/_static/image44.png "验证连接")</span><span class="sxs-lookup"><span data-stu-id="16f1e-611">![Validating connection](whats-new-in-web-forms-in-aspnet-45/_static/image44.png "Validating connection")</span></span>

    <span data-ttu-id="16f1e-612">*正在验证连接*</span><span class="sxs-lookup"><span data-stu-id="16f1e-612">*Validating connection*</span></span>
4. <span data-ttu-id="16f1e-613">在**设置**页上，在**数据库**部分中，单击你的数据库连接的文本框旁边的按钮 (即**DefaultConnection**)。</span><span class="sxs-lookup"><span data-stu-id="16f1e-613">In the **Settings** page, under the **Databases** section, click the button next to your database connection's textbox (i.e. **DefaultConnection**).</span></span>

    <span data-ttu-id="16f1e-614">![Web 部署配置](whats-new-in-web-forms-in-aspnet-45/_static/image45.png "Web 部署配置")</span><span class="sxs-lookup"><span data-stu-id="16f1e-614">![Web deploy configuration](whats-new-in-web-forms-in-aspnet-45/_static/image45.png "Web deploy configuration")</span></span>

    <span data-ttu-id="16f1e-615">*Web 部署配置*</span><span class="sxs-lookup"><span data-stu-id="16f1e-615">*Web deploy configuration*</span></span>
5. <span data-ttu-id="16f1e-616">配置数据库连接，如下所示：</span><span class="sxs-lookup"><span data-stu-id="16f1e-616">Configure the database connection as follows:</span></span>

   - <span data-ttu-id="16f1e-617">在**服务器名称**类型 SQL 数据库服务器 URL 使用*tcp:*前缀。</span><span class="sxs-lookup"><span data-stu-id="16f1e-617">In the **Server name** type your SQL Database server URL using the *tcp:* prefix.</span></span>
   - <span data-ttu-id="16f1e-618">在**用户名**键入您的服务器管理员登录名。</span><span class="sxs-lookup"><span data-stu-id="16f1e-618">In **User name** type your server administrator login name.</span></span>
   - <span data-ttu-id="16f1e-619">在**密码**键入服务器管理员登录密码。</span><span class="sxs-lookup"><span data-stu-id="16f1e-619">In **Password** type your server administrator login password.</span></span>
   - <span data-ttu-id="16f1e-620">键入新的数据库名称。</span><span class="sxs-lookup"><span data-stu-id="16f1e-620">Type a new database name.</span></span>

     <span data-ttu-id="16f1e-621">![配置目标连接字符串](whats-new-in-web-forms-in-aspnet-45/_static/image46.png "配置目标连接字符串")</span><span class="sxs-lookup"><span data-stu-id="16f1e-621">![Configuring destination connection string](whats-new-in-web-forms-in-aspnet-45/_static/image46.png "Configuring destination connection string")</span></span>

     <span data-ttu-id="16f1e-622">*配置目标连接字符串*</span><span class="sxs-lookup"><span data-stu-id="16f1e-622">*Configuring destination connection string*</span></span>
6. <span data-ttu-id="16f1e-623">然后单击“确定” 。</span><span class="sxs-lookup"><span data-stu-id="16f1e-623">Then click **OK**.</span></span> <span data-ttu-id="16f1e-624">当系统提示创建数据库单击**是**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-624">When prompted to create the database click **Yes**.</span></span>

    <span data-ttu-id="16f1e-625">![创建数据库](whats-new-in-web-forms-in-aspnet-45/_static/image47.png "创建数据库字符串")</span><span class="sxs-lookup"><span data-stu-id="16f1e-625">![Creating the database](whats-new-in-web-forms-in-aspnet-45/_static/image47.png "Creating the database string")</span></span>

    <span data-ttu-id="16f1e-626">*创建数据库*</span><span class="sxs-lookup"><span data-stu-id="16f1e-626">*Creating the database*</span></span>
7. <span data-ttu-id="16f1e-627">在默认连接文本框中显示了将用于连接到 Azure 中的 SQL 数据库连接字符串。</span><span class="sxs-lookup"><span data-stu-id="16f1e-627">The connection string you will use to connect to SQL Database in Azure is shown within Default Connection textbox.</span></span> <span data-ttu-id="16f1e-628">然后，单击 **“下一步”**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-628">Then click **Next**.</span></span>

    <span data-ttu-id="16f1e-629">![连接字符串指向 SQL 数据库](whats-new-in-web-forms-in-aspnet-45/_static/image48.png "指向 SQL 数据库的连接字符串")</span><span class="sxs-lookup"><span data-stu-id="16f1e-629">![Connection string pointing to SQL Database](whats-new-in-web-forms-in-aspnet-45/_static/image48.png "Connection string pointing to SQL Database")</span></span>

    <span data-ttu-id="16f1e-630">*连接字符串指向 SQL 数据库*</span><span class="sxs-lookup"><span data-stu-id="16f1e-630">*Connection string pointing to SQL Database*</span></span>
8. <span data-ttu-id="16f1e-631">在**预览**页上，单击**发布**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-631">In the **Preview** page, click **Publish**.</span></span>

    <span data-ttu-id="16f1e-632">![Web 应用程序发布](whats-new-in-web-forms-in-aspnet-45/_static/image49.png "发布 web 应用程序")</span><span class="sxs-lookup"><span data-stu-id="16f1e-632">![Publishing the web application](whats-new-in-web-forms-in-aspnet-45/_static/image49.png "Publishing the web application")</span></span>

    <span data-ttu-id="16f1e-633">*发布 web 应用程序*</span><span class="sxs-lookup"><span data-stu-id="16f1e-633">*Publishing the web application*</span></span>
9. <span data-ttu-id="16f1e-634">完成发布过程后，默认浏览器将打开已发布的网站。</span><span class="sxs-lookup"><span data-stu-id="16f1e-634">Once the publishing process finishes, your default browser will open the published web site.</span></span>

<a id="AppendixC"></a>

<a id="Appendix_C_Using_Code_Snippets"></a>
## <a name="appendix-c-using-code-snippets"></a><span data-ttu-id="16f1e-635">附录 c： 使用代码段</span><span class="sxs-lookup"><span data-stu-id="16f1e-635">Appendix C: Using Code Snippets</span></span>

<span data-ttu-id="16f1e-636">和代码片段，必须将你能够轻松获得所需的所有代码所示。</span><span class="sxs-lookup"><span data-stu-id="16f1e-636">With code snippets, you have all the code you need at your fingertips.</span></span> <span data-ttu-id="16f1e-637">实验室文档将告诉您完全时你可以使用它们，如下图中所示。</span><span class="sxs-lookup"><span data-stu-id="16f1e-637">The lab document will tell you exactly when you can use them, as shown in the following figure.</span></span>

<span data-ttu-id="16f1e-638">![使用 Visual Studio 代码段将代码插入到你的项目](whats-new-in-web-forms-in-aspnet-45/_static/image50.png "使用 Visual Studio 代码片段，可将代码插入到你的项目")</span><span class="sxs-lookup"><span data-stu-id="16f1e-638">![Using Visual Studio code snippets to insert code into your project](whats-new-in-web-forms-in-aspnet-45/_static/image50.png "Using Visual Studio code snippets to insert code into your project")</span></span>

<span data-ttu-id="16f1e-639">*使用 Visual Studio 代码段将代码插入到你的项目*</span><span class="sxs-lookup"><span data-stu-id="16f1e-639">*Using Visual Studio code snippets to insert code into your project*</span></span>

<span data-ttu-id="16f1e-640">***若要添加代码片段使用键盘 (仅限 C#)***</span><span class="sxs-lookup"><span data-stu-id="16f1e-640">***To add a code snippet using the keyboard (C# only)***</span></span>

1. <span data-ttu-id="16f1e-641">将光标置于想要插入代码。</span><span class="sxs-lookup"><span data-stu-id="16f1e-641">Place the cursor where you would like to insert the code.</span></span>
2. <span data-ttu-id="16f1e-642">开始键入代码段名称 （不带空格或连字符）。</span><span class="sxs-lookup"><span data-stu-id="16f1e-642">Start typing the snippet name (without spaces or hyphens).</span></span>
3. <span data-ttu-id="16f1e-643">观看作为 IntelliSense 显示匹配的代码段的名称。</span><span class="sxs-lookup"><span data-stu-id="16f1e-643">Watch as IntelliSense displays matching snippets' names.</span></span>
4. <span data-ttu-id="16f1e-644">选择正确的代码段 （或继续键入直到选中整个代码段的名称）。</span><span class="sxs-lookup"><span data-stu-id="16f1e-644">Select the correct snippet (or keep typing until the entire snippet's name is selected).</span></span>
5. <span data-ttu-id="16f1e-645">按 Tab 键两次以光标位置处插入代码段。</span><span class="sxs-lookup"><span data-stu-id="16f1e-645">Press the Tab key twice to insert the snippet at the cursor location.</span></span>

<span data-ttu-id="16f1e-646">![开始键入代码段名称](whats-new-in-web-forms-in-aspnet-45/_static/image51.png "开始键入代码段名称")</span><span class="sxs-lookup"><span data-stu-id="16f1e-646">![Start typing the snippet name](whats-new-in-web-forms-in-aspnet-45/_static/image51.png "Start typing the snippet name")</span></span>

<span data-ttu-id="16f1e-647">*开始键入代码段名称*</span><span class="sxs-lookup"><span data-stu-id="16f1e-647">*Start typing the snippet name*</span></span>

<span data-ttu-id="16f1e-648">![按 Tab 以选择突出显示代码段](whats-new-in-web-forms-in-aspnet-45/_static/image52.png "按选项卡以选择突出显示代码段")</span><span class="sxs-lookup"><span data-stu-id="16f1e-648">![Press Tab to select the highlighted snippet](whats-new-in-web-forms-in-aspnet-45/_static/image52.png "Press Tab to select the highlighted snippet")</span></span>

<span data-ttu-id="16f1e-649">*按 Tab 以选择突出显示代码段*</span><span class="sxs-lookup"><span data-stu-id="16f1e-649">*Press Tab to select the highlighted snippet*</span></span>

<span data-ttu-id="16f1e-650">![再次按 Tab 和代码段将会扩展](whats-new-in-web-forms-in-aspnet-45/_static/image53.png "再次按 Tab 和代码段将会扩展")</span><span class="sxs-lookup"><span data-stu-id="16f1e-650">![Press Tab again and the snippet will expand](whats-new-in-web-forms-in-aspnet-45/_static/image53.png "Press Tab again and the snippet will expand")</span></span>

<span data-ttu-id="16f1e-651">*再次按 Tab 和代码段将会扩展*</span><span class="sxs-lookup"><span data-stu-id="16f1e-651">*Press Tab again and the snippet will expand*</span></span>

<span data-ttu-id="16f1e-652">***若要添加代码片段使用鼠标 （C#、 Visual Basic 和 XML）*** 1。</span><span class="sxs-lookup"><span data-stu-id="16f1e-652">***To add a code snippet using the mouse (C#, Visual Basic and XML)*** 1.</span></span> <span data-ttu-id="16f1e-653">右键单击你想要插入代码段。</span><span class="sxs-lookup"><span data-stu-id="16f1e-653">Right-click where you want to insert the code snippet.</span></span>

1. <span data-ttu-id="16f1e-654">选择**插入代码段**跟**我的代码段**。</span><span class="sxs-lookup"><span data-stu-id="16f1e-654">Select **Insert Snippet** followed by **My Code Snippets**.</span></span>
2. <span data-ttu-id="16f1e-655">选择相关的代码段从列表中，通过单击它。</span><span class="sxs-lookup"><span data-stu-id="16f1e-655">Pick the relevant snippet from the list, by clicking on it.</span></span>

<span data-ttu-id="16f1e-656">![右键单击你想要插入代码段，并选择插入代码段](whats-new-in-web-forms-in-aspnet-45/_static/image54.png "右键单击你想要插入代码段，并选择插入代码段")</span><span class="sxs-lookup"><span data-stu-id="16f1e-656">![Right-click where you want to insert the code snippet and select Insert Snippet](whats-new-in-web-forms-in-aspnet-45/_static/image54.png "Right-click where you want to insert the code snippet and select Insert Snippet")</span></span>

<span data-ttu-id="16f1e-657">*右键单击你想要插入代码段，并选择插入代码段*</span><span class="sxs-lookup"><span data-stu-id="16f1e-657">*Right-click where you want to insert the code snippet and select Insert Snippet*</span></span>

<span data-ttu-id="16f1e-658">![通过单击它选取列表中中的相关代码片段](whats-new-in-web-forms-in-aspnet-45/_static/image55.png "选取相关的代码段从列表中，通过单击它")</span><span class="sxs-lookup"><span data-stu-id="16f1e-658">![Pick the relevant snippet from the list, by clicking on it](whats-new-in-web-forms-in-aspnet-45/_static/image55.png "Pick the relevant snippet from the list, by clicking on it")</span></span>

<span data-ttu-id="16f1e-659">*通过单击它选取从列表中，相关代码段*</span><span class="sxs-lookup"><span data-stu-id="16f1e-659">*Pick the relevant snippet from the list, by clicking on it*</span></span>
