...
这就是一切就这么简单 ！ 在应用程序启动时，`LoadStaticCache()`方法将获取从 BLL，供应商信息，并将其存储在静态成员变量 (或任何缓存存储您最终会在中使用`StaticCache`类)。 若要验证此行为中, 设置断点`Application_Start`方法并运行应用程序。 请注意应用程序启动时命中断点。 后续请求中，但是，不会导致`Application_Start`要执行的方法。
