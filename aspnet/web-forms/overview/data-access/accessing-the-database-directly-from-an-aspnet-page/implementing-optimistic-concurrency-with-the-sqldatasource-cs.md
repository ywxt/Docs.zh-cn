...
**图 2**： 当两个用户同时更新的记录那里 s 可能有一个用户 s 将更改为覆盖其他 s ([单击以查看实际尺寸的图像](implementing-optimistic-concurrency-with-the-sqldatasource-cs/_static/image2.png))
...
**图 3**: For Update 或 Delete succeed，原始值必须为等于当前的数据库值 ([单击以查看实际尺寸的图像](implementing-optimistic-concurrency-with-the-sqldatasource-cs/_static/image4.png))
...
**图 5**： 从`Products`表中，返回`ProductID`， `ProductName`， `UnitPrice`，并且`Discontinued`列 ([单击以查看实际尺寸的图像](implementing-optimistic-concurrency-with-the-sqldatasource-cs/_static/image8.png))
...
**图 8**： 将标签控件添加到要显示该警告的页面 ([单击以查看实际尺寸的图像](implementing-optimistic-concurrency-with-the-sqldatasource-cs/_static/image14.png))
...
**图 9**： 在遇到并发冲突时显示消息 ([单击以查看实际尺寸的图像](implementing-optimistic-concurrency-with-the-sqldatasource-cs/_static/image16.png))
