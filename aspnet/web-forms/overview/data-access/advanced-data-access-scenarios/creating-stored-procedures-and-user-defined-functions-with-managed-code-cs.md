...
ms.locfileid: "37388947"
---
<a name="creating-stored-procedures-and-user-defined-functions-with-managed-code-c"></a>创建存储过程和用户定义函数使用托管代码 (C#)
====================
...
**图 12**: clr 已启用设置是当前设置为 0 ([单击以查看实际尺寸的图像](creating-stored-procedures-and-user-defined-functions-with-managed-code-cs/_static/image22.png))
...
**图 21**： 选择`GetDiscontinuedProducts`从选择选项卡中的下拉列表的方法 ([单击以查看实际尺寸的图像](creating-stored-procedures-and-user-defined-functions-with-managed-code-cs/_static/image49.png))
...
**图 22**： 列出了停止使用的产品 ([单击以查看实际尺寸的图像](creating-stored-procedures-and-user-defined-functions-with-managed-code-cs/_static/image52.png))
...
**图 23**： 每个产品 s 列出清单值 ([单击以查看实际尺寸的图像](creating-stored-procedures-and-user-defined-functions-with-managed-code-cs/_static/image55.png))
...
**图 24**: `ProductID`， `ProductName`，和`CategoryID`列出的每个饮料 ([单击以查看实际尺寸的图像](creating-stored-procedures-and-user-defined-functions-with-managed-code-cs/_static/image58.png))
...
在中[调试存储过程](debugging-stored-procedures-cs.md)调试通过 Visual Studio 的 SQL Server 教程中，我们讨论了这三个选项： 直接数据库调试、 应用程序调试和从 SQL Server 项目的调试。 托管的数据库对象不能通过直接数据库调试调试，但可以进行调试，从客户端应用程序和直接从 SQL Server 项目。 为了使调试工作，但是，SQL Server 2005 数据库必须允许 SQL/CLR 调试。 回想一下，我们首次创建时`ManagedDatabaseConstructs`Visual Studio 要求我们是否我们想要启用 SQL/CLR 调试 （见图 6 在步骤 2 中） 的项目。 可以通过右键单击服务器资源管理器窗口中的数据库修改此设置。
