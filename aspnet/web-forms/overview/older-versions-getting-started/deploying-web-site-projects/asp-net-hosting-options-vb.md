...
步骤 2 涉及到同步组基本的 ASP.NET 页面和开发和生产环境之间的支持文件。 这组特定的 ASP。提供与.NET 相关的文件需要两个环境之间同步取决于你在 Visual Studio 中，创建并在下一步的教程中，讨论才具有的项目的类型<em>[确定文件需要什么部署到](determining-what-files-need-to-be-deployed-vb.md)</em>. 第三个和第四个教程- <em>[部署你的站点使用 FTP](deploying-your-site-using-an-ftp-client-vb.md)</em>和<em>[部署您站点使用 Visual Studio](deploying-your-site-using-visual-studio-vb.md)</em>  -检查不同的工具和技术用于这些文件的同步。
