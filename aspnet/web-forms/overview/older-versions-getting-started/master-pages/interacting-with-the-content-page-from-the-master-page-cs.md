...
*发布服务器*是对提供事件的对象的引用*eventName*，和*methodName*是在订阅服务器上具有对应的签名中定义的事件处理程序的名称*eventDelegate*。 换而言之，如果事件委托是`EventHandler`，然后*methodName*必须是不返回值并接受两个输入参数的类型的订阅服务器中的方法的名称`Object`和`EventArgs`，分别。
