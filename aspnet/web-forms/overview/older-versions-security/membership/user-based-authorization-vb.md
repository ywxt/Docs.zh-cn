...
> [!NOTE]
> `DirectoryInfo`和`FileInfo`类位于[`System.IO`命名空间](https://msdn.microsoft.com/library/system.io.aspx)。 因此，将任一需要开头这些类名称与命名空间名称或具有到的类文件导入的命名空间 (通过`Imports System.IO`)。
